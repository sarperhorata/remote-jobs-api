...
class NotificationManager:
    ...
    async def send_notification(
        self, notification: Notification, change_type: str, data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            if notification.notification_type == NotificationType.EMAIL:
                ...
                logger.error(
                    f"Unsupported notification type: {notification.notification_type}"
                )
                return False
    ...
    async def _send_webhook_notification(
        ...
        try:
            ...
            response = requests.post(
                webhook_url, json=payload, headers=self.headers, timeout=30
            )
            response.raise_for_status()
    ...
    def _create_notification_content(
        self, change_type: str, data: Dict[str, Any], is_html: bool = True
    ) -> tuple:
        """
    ...
    async def send_deployment_notification(self, deployment_info: dict):
        ...
        try:
            ...
            async with self.db.session() as session:
                users = (
                    await session.query(UserNotificationPreference)
                    .filter(UserNotificationPreference.notify_on_deployment == True)
                    .all()
                )
