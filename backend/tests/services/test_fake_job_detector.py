...
import os
from services.fake_job_detector import FakeJobDetector, FakeJobAnalysis
...
class TestFakeJobDetector:
    ...
    def fake_job_detector_with_openai(self, mock_openai_client):
        ...
        with patch.dict(os.environ, {'OPENAI_API_KEY': 'test_key'}):
            with patch('services.fake_job_detector.openai.OpenAI', return_value=mock_openai_client):
    ...
    def test_service_initialization_without_openai(self, fake_job_detector_without_openai):
    ...
    @pytest.mark.asyncio
    async def test_analyze_job_success(self, fake_job_detector_with_openai, sample_job_data):
        ...
        mock_response.choices[0].message.content = '{"is_fake": false, "confidence": 0.9, "reasons": ["Legitimate company"], "risk_score": 0.1}'
    ...
    @pytest.mark.asyncio
    async def test_analyze_fake_job(self, fake_job_detector_with_openai, sample_fake_job_data):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = '{"is_fake": true, "confidence": 0.95, "reasons": ["Unrealistic salary", "Suspicious contact"], "risk_score": 0.9}'
        detector.client.chat.completions.create.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_analyze_job_without_openai(self, fake_job_detector_without_openai, sample_job_data):
    ...
    @pytest.mark.asyncio
    async def test_analyze_job_error(self, fake_job_detector_with_openai, sample_job_data):
        """Job analizi hatası testi"""
        detector = fake_job_detector_with_openai
        
        # Mock AI error
        detector.client.chat.completions.create.side_effect = Exception("OpenAI API error")
    ...
    def test_analyze_with_rules_legitimate_job(self, fake_job_detector_with_openai, sample_job_data):
    ...
    def test_analyze_with_rules_fake_job(self, fake_job_detector_with_openai, sample_fake_job_data):
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_ai_success(self, fake_job_detector_with_openai, sample_job_data):
        """AI analizi başarılı testi"""
        detector = fake_job_detector_with_openai
        
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = '{"is_fake": false, "confidence": 0.9, "reasons": ["Legitimate"], "risk_score": 0.1}'
        detector.client.chat.completions.create.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_ai_error(self, fake_job_detector_with_openai, sample_job_data):
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_ai_invalid_json(self, fake_job_detector_with_openai, sample_job_data):
    ...
    @pytest.mark.asyncio
    async def test_service_integration(self, fake_job_detector_with_openai, sample_job_data, sample_fake_job_data):
        ...
        # Test fake job
        mock_response.choices[0].message.content = '{"is_fake": true, "confidence": 0.95, "reasons": ["Fake"], "risk_score": 0.9}'
