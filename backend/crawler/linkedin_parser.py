...
class LinkedInParser:
    ...
    def get_jobs(
        ...
        try:
            params = {
                ...
                "count": min(
                    limit, 100
                ),  # LinkedIn typically limits to 100 per request
            }
    ...
    def get_job_details(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if json_ld:
                job_details.update(
                    {
                        "date_posted": json_ld.get(
                            "datePosted", job_details.get("date_retrieved")
                        ),
    ...
    def paginate_jobs(
        self, keywords: str = "remote", location: str = "", total_count: int = 100
    ) -> List[Dict[str, Any]]:
        ...
        while len(all_jobs) < total_count:
            jobs = self.get_jobs(
                keywords=keywords, location=location, start=start, limit=page_size
            )
