...
class PreDeploymentTester:
    ...
    def _check_syntax(self) -> bool:
        ...
        syntax_test_file = self.backend_dir / "tests" / "syntax" / "test_syntax_and_imports.py"
    ...
    def _run_frontend_tests(self) -> bool:
        ...
        # Run tests
        test_cmd = ["npm", "test", "--", "--coverage", "--watchAll=false", "--passWithNoTests"]
        ...
        if coverage_file.exists():
            try:
                with open(coverage_file) as f:
                    coverage_data = json.load(f)
                    coverage_percent = coverage_data.get("total", {}).get("lines", {}).get("pct", 0)
            except Exception as e:
        ...
        self.test_results["frontend"] = {
            "passed": result.returncode == 0,
            "coverage": coverage_percent,
            "meets_threshold": coverage_percent >= (self.coverage_threshold - 5)  # Slightly lower threshold for frontend
        }
        ...
        if coverage_percent < frontend_threshold:
            logger.error(f"❌ Frontend coverage {coverage_percent:.1f}% below threshold {frontend_threshold}%")
            return False
        ...
        try:
            ...
            for pattern in patterns:
                result = subprocess.run([
                    "grep", "-r", "-E", pattern, str(self.backend_dir / "routes"),
        ...
        try:
            ...
            if main_file.exists():
                ...
                if "allow_origins=[\"*\"]" in content:
                    logger.warning("⚠️ CORS allows all origins - check if this is intended for production")
            return True
        except Exception:
            return True
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if backend.get("coverage", 0) < 90:
            recommendations.append("Consider increasing backend test coverage above 90%")
        
        if frontend.get("coverage", 0) < 85:
            recommendations.append("Consider increasing frontend test coverage above 85%")
