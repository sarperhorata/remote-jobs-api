...
class LinkedInIntegration:
    ...
    def get_user_profile(self, access_token: str) -> Optional[Dict]:
        ...
        try:
            ...
            if profile_response.status_code == 200:
                ...
                email_data = (
                    email_response.json() if email_response.status_code == 200 else {}
                )
    ...
    def _format_profile_data(self, profile_data: Dict, email_data: Dict) -> Dict:
        ...
        if email_data.get("elements"):
            ...
            formatted_data["email"] = email_element.get("handle~", {}).get(
                "emailAddress", ""
            )
        ...
        if "profilePicture" in profile_data:
            ...
            if display_image.get("elements"):
                largest_image = max(
                    display_image["elements"],
                    key=lambda x: x.get("data", {})
                    .get("com.linkedin.digitalmedia.mediaartifact.StillImage", {})
                    .get("storageSize", {})
                    .get("width", 0),
                )
                formatted_data["profile_photo_url"] = largest_image.get(
                    "identifiers", [{}]
                )[0].get("identifier", "")
...
def fetch_linkedin_data(linkedin_url: str) -> Dict:
    ...
    try:
        ...
        return {"name": "", "title": "", "linkedin_url": linkedin_url}
    except Exception as e:
