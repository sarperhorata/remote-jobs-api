...
from backend.models.user_activity import (ActivitySummary, ActivityType,
...
class ActivityLogger:
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.db.activity_summaries.create_index(
                [("user_id", 1), ("date", -1)]
            )
            await self.db.activity_summaries.create_index(
            ...
            await self.db.user_activities.create_index(
                [("timestamp", 1)], expireAfterSeconds=30 * 24 * 60 * 60  # 30 days
            )
    ...
    async def log_activity(
        ...
        **kwargs,
        ...
        try:
            ...
            # Check if we're in test environment with mock database
            if hasattr(self.db, "_MockDatabase__name") or "MockDatabase" in str(
                type(self.db)
            ):
    ...
    async def start_session(
        self, user_id: str, session_token: str, request_data: Dict[str, Any]
    ) -> str:
    ...
    async def _update_session_activity(
        self, session_id: str, activity_type: ActivityType
    ):
        ...
        try:
            ...
            await self.db.user_sessions.update_one({"_id": session_id}, update_data)
        except Exception as e:
    ...
    async def _update_activity_summary(self, user_id: str, activity_type: ActivityType):
        ...
        try:
            ...
            await self.db.activity_summaries.update_one(
                summary_filter, update_data, upsert=True
            )
    ...
    async def get_user_activities(
        ...
        try:
            ...
            # Execute query
            cursor = (
                self.db.user_activities.find(query)
                .sort("timestamp", -1)
                .skip(skip)
                .limit(limit)
            )
    ...
    async def get_user_sessions(
        self, user_id: str, limit: int = 20, skip: int = 0, active_only: bool = False
    ) -> List[Dict[str, Any]]:
        """Get user sessions"""
        try:
            if self.db is None:
            ...
            cursor = (
                self.db.user_sessions.find(query)
                .sort("started_at", -1)
                .skip(skip)
                .limit(limit)
            )
    ...
    async def get_activity_analytics(
        ...
        try:
            ...
            results = await self.db.user_activities.aggregate(pipeline).to_list(
                length=None
            )
