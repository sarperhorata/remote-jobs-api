...
async def test_payment_timestamp_validation():
    ...
    assert future_timestamp > current_timestamp, "Future timestamp should be greater than current"
    
    # Test past timestamp (should be valid)
    past_timestamp = int((datetime.now() - timedelta(hours=1)).timestamp())
    assert past_timestamp < current_timestamp, "Past timestamp should be less than current"
...
async def test_payment_error_handling():
    ...
    assert not isinstance(invalid_payment["amount"], int), "Amount should not be integer"
    assert not isinstance(invalid_payment["created"], int), "Created should not be integer"
