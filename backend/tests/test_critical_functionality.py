...
async def test_user_data_validation():
    ...
    ), "Password should contain special char"
    ...
    def search_jobs(query, job_list):
        ...
        for job in job_list:
            if (
                ...
                or any(query_lower in skill.lower() for skill in job["skills"])
            ):
                results.append(job)
        return results
    ...
    def filter_jobs(job_list, **filters):
        ...
        for key, value in filters.items():
            if key == "salary_min":
                ...
                filtered = [
                    j for j in filtered if j["location"].lower() == value.lower()
                ]
            elif key == "experience":
...
async def test_data_sorting():
    ...
    assert (
        title_asc[0]["title"] == "DevOps Engineer"
    ), "Alphabetically first title should be first"
...
async def test_error_handling():
    ...
    assert (
        safe_get_nested(data, ["user", "profile", "name"]) == "John"
    ), "Valid path should work"
    assert (
        safe_get_nested(data, ["user", "profile", "age"]) is None
    ), "Invalid key should return None"
    assert (
        safe_get_nested(data, ["user", "invalid"]) is None
...
async def test_business_rules():
    ...
    assert (
        "Minimum salary cannot exceed maximum salary" in errors
    ), "Should catch salary range error"
