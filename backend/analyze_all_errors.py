...
async def analyze_all_errors():
    try:
        ...
        latest_log = await db.processing_logs.find_one(
            {"type": "company_website_enrichment"}, sort=[("timestamp", -1)]
        )
        ...
        if latest_log:
            ...
            print("-" * 50)
            print(
                "(These are companies successfully crawled but not found in jobs database)"
            )
            ...
            print(
                f'Success rate: {(stats.get("updated_companies", 0) / stats.get("total_companies", 1) * 100):.1f}%'
            )
            ...
            if processing_errors:
                ...
                print(
                    "The 230 errors are only company name matching issues in database."
                )
