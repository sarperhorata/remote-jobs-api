...
class SalaryEstimationService:
    ...
    def _extract_currency(self, text: str) -> str:
        ...
        currencies = ['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'CHF', 'SEK', 'NOK', 'DKK', 'PLN', 'CZK', 'HUF', 'RON', 'BGN', 'HRK', 'RSD', 'MKD', 'ALL', 'BAM', 'MDL', 'UAH', 'GEL', 'AMD', 'AZN', 'BYN', 'KZT', 'KGS', 'TJS', 'TMT', 'UZS', 'MNT', 'LAK', 'KHR', 'MMK', 'THB', 'VND', 'IDR', 'MYR', 'SGD', 'BND', 'PHP', 'INR', 'PKR', 'BDT', 'LKR', 'NPR', 'BTN', 'MVR', 'AED', 'QAR', 'SAR', 'OMR', 'KWD', 'BHD', 'JOD', 'ILS', 'EGP', 'LYD', 'TND', 'DZD', 'MAD', 'MRO', 'XOF', 'XAF', 'XPF', 'GHS', 'NGN', 'KES', 'UGX', 'TZS', 'MWK', 'ZMW', 'ZAR', 'BWP', 'NAD', 'SZL', 'LSL', 'MUR', 'SCR', 'KMF', 'DJF', 'ETB', 'SOS', 'SDG', 'SSP', 'CDF', 'RWF', 'BIF', 'GMD', 'GNF', 'SLL', 'LRD', 'SLE', 'GIP', 'FKP', 'SHP', 'AOA', 'STD', 'CVE', 'GQE', 'XCD', 'BBD', 'TTD', 'JMD', 'HTG', 'GYD', 'SRD', 'BZD', 'BMD', 'KYD', 'AWG', 'ANG', 'XPF', 'TOP', 'WST', 'FJD', 'VUV', 'SBD', 'PGK']
    ...
    def _determine_period(self, text: str) -> str:
        ...
        if any(word in text for word in ['yearly', 'annual', 'annually', 'per year', 'yr']):
            return 'yearly'
    ...
    async def find_similar_jobs(self, job_title: str, location: str = None, company_size: str = None, experience_level: str = None) -> List[Dict]:
        """Benzer işleri bul"""
        if self.db is None:
        ...
        try:
            # Benzer işleri bul
            similar_jobs = await self.db.jobs.find(search_criteria).limit(50).to_list(50)
    ...
    async def estimate_salary(self, job_title: str, location: str = None, company_size: str = None, experience_level: str = None) -> Optional[Dict]:
        """Maaş tahmini yap"""
        try:
            # Benzer işleri bul
            similar_jobs = await self.find_similar_jobs(job_title, location, company_size, experience_level)
            ...
            for job in similar_jobs:
                ...
                if min_salary and max_salary:
                    ...
                    normalized_min = self.normalize_salary_to_yearly(min_salary, job.get('salary_period', 'yearly'))
                    normalized_max = self.normalize_salary_to_yearly(max_salary, job.get('salary_period', 'yearly'))
            ...
            same_currency_salaries = [s for s in salaries if s['currency'] == most_common_currency]
            ...
            filtered_salaries = [s for s in salary_values_same_currency if lower_bound <= s <= upper_bound]
            ...
            if filtered_salaries:
                ...
                return {
                    ...
                    'period': 'yearly',
                    'is_estimated': True,
                    'confidence_score': self._calculate_confidence_score(len(filtered_salaries), len(similar_jobs)),
