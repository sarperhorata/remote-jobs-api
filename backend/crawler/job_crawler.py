...
from models.models import Job, SelectorBase, Website, WebsiteType
...
class JobCrawler:
    ...
    async def get_jobs_from_website(
        self,
        website: Website,
        ...
        try:
            ...
            for job in job_listings:
                try:
                    ...
                    tags = (
                        [tag.text.strip() for tag in tags_elems] if tags_elems else []
                    )
    ...
    async def _crawl_jobs_from_space(self, website: Website) -> List[Job]:
        ...
        try:
            ...
            job_listings_container = soup.find(
                "div", class_="job-listings"
            ) or soup.find("div", {"id": "job-list"})
            ...
            if not job_listings_container:
                job_listings = soup.find_all(
                    "div",
                    class_=lambda c: c
                    and ("job-card" in c.lower() or "job-listing" in c.lower()),
                )
            else:
                job_listings = job_listings_container.find_all(
                    "div", class_=lambda c: c and ("job" in c.lower())
                )
            ...
            for job in job_listings:
                try:
                    ...
                    title_elem = (
                        job.find("h2")
                        or job.find("h3")
                        or job.find("a", class_=lambda c: c and ("title" in c.lower()))
                    )
                    ...
                    company_elem = job.find(
                        ...
                        "div", class_=lambda c: c and ("company" in c.lower())
                    )
                    ...
                    location_elem = job.find(
                        ...
                        "div", class_=lambda c: c and ("location" in c.lower())
                    )

                    # Extract career page URL
                    career_page_elem = job.find(
                        "a",
                        href=True,
                        text=lambda t: t
                        and ("career" in t.lower() or "jobs" in t.lower()),
                    ) or job.find(
                        "a",
                        href=True,
                        class_=lambda c: c
                        and ("career" in c.lower() or "jobs" in c.lower()),
                    )
                    career_page_url = (
                        career_page_elem["href"] if career_page_elem else None
                    )
                    if career_page_url and not career_page_url.startswith(
                        ("http://", "https://")
                    ):
                        career_page_url = urljoin(str(website.url), career_page_url)

                    # Extract tags/skills
                    tags_container = job.find(
                        "div",
                        class_=lambda c: c
                        and ("tags" in c.lower() or "skills" in c.lower()),
                    )
                    tags = []
                    if tags_container:
                        tag_elements = tags_container.find_all(
                            "span"
                        ) or tags_container.find_all("a")
                    ...
                    if not url.startswith(("http://", "https://")):
                        url = (
                            f"https://www.jobsfromspace.com{url}"
                            if not url.startswith("/")
                            else f"https://www.jobsfromspace.com{url}"
                        )
    ...
    async def _crawl_custom_website(self, website: Website) -> List[Job]:
        ...
        try:
            ...
            job_container_selector = next(
                (s for s in website.selectors if s.name == "job_container"), None
            )
            title_selector = next(
                (s for s in website.selectors if s.name == "title"), None
            )
            company_selector = next(
                (s for s in website.selectors if s.name == "company"), None
            )
            url_selector = next((s for s in website.selectors if s.name == "url"), None)
            location_selector = next(
                (s for s in website.selectors if s.name == "location"), None
            )
            ...
            for job_element in job_elements:
                try:
                    ...
                    company = (
                        self._extract_with_selector(job_element, company_selector)
                        if company_selector
                        else "Unknown"
                    )
                        ...
                        if url_elem is not None:
                            ...
                            if hasattr(url_elem, "get") and hasattr(url_elem, "name"):
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Could not extract URL from element: {type(url_elem)}"
                                    )
                                    url = ""
                    ...
                    # Extract location
                    location = (
                        self._extract_with_selector(job_element, location_selector)
                        if location_selector
                        else None
                    )

                    # Extract career page URL
                    career_page_elem = job_element.find(
                        "a",
                        href=True,
                        text=lambda t: t
                        and ("career" in t.lower() or "jobs" in t.lower()),
                    ...
                    logger.error(
                        f"Error parsing job from custom website {website.name}: {e}"
                    )
    ...
    def filter_jobs_by_keywords(
        ...
        for job in jobs:
            ...
            if exclude_keywords:
                exclude_match = any(
                    keyword.lower() in job_text for keyword in exclude_keywords
                )
    ...
    async def get_job_details(self, job_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"description": description, "salary": salary, "raw_data": str(soup)}

        except Exception as e:
            logger.error(f"Error fetching job details for {job_url}: {e}")
            return {"description": "", "salary": None, "raw_data": None}
                ...
                try:
                    ...
                    job_data = {
                        ...
                        "posted_date": datetime.fromisoformat(
                            job.get("publication_date", datetime.now().isoformat())
                        ),
                        "raw_data": job,
                    }
