...
class TestUserApplicationService:
    ...
    def sample_application_response(self, sample_application_data):
        ...
        return UserApplicationResponse(id=str(ObjectId()), **sample_application_data)
    ...
    async def test_create_application_success(
        self, service, sample_application_create, sample_application_response
    ):
        ...
        service.collection.find_one = AsyncMock(
            return_value=sample_application_response.model_dump()
        )
    ...
    async def test_create_application_database_error(
        self, service, sample_application_create
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_application_retrieve_error(
        self, service, sample_application_create
    ):
    ...
    async def test_get_application_by_id_success(
        self, service, sample_application_response
    ):
    ...
    async def test_search_applications(self, service, sample_application_response):
        ...
        result = await service.search_applications(
            "user123", search_filters, skip=0, limit=10
        )
    ...
    async def test_get_applications_by_status(
        ...
        result = await service.get_applications_by_status(
            "user123", "applied", skip=0, limit=10
        )

        assert len(result) == 1
        assert isinstance(result[0], UserApplicationResponse)
        service.collection.find.assert_called_once_with(
            {"user_id": "user123", "status": "applied"}
        )
