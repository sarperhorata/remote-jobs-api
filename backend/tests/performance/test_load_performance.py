...
class TestLoadPerformance:
    ...
    def test_single_request_performance(self, client):
        ...
        for endpoint in endpoints:
            ...
            assert (
                avg_time < 2.0
            ), f"Average response time too slow for {endpoint}: {avg_time:.3f}s"
            assert (
                max_time < 5.0
            ), f"Max response time too slow for {endpoint}: {max_time:.3f}s"
    ...
    def test_concurrent_requests(self, client):
        ...
        assert (
            total_time < 10.0
        ), f"Total concurrent test time too slow: {total_time:.3f}s"
        assert (
            avg_response_time < 1.0
        ), f"Average concurrent response time too slow: {avg_response_time:.3f}s"
    ...
    def test_database_query_performance(self, client):
        ...
        for query in queries:
            ...
            assert (
                avg_time < 3.0
            ), f"Database query too slow for {query}: {avg_time:.3f}s"
    ...
    def test_pagination_performance(self, client):
        ...
        for limit in page_sizes:
            ...
            if limit <= 25:
                assert (
                    avg_time < 2.0
                ), f"Pagination too slow for limit {limit}: {avg_time:.3f}s"
            else:
                assert (
                    avg_time < 3.0
                ), f"Pagination too slow for limit {limit}: {avg_time:.3f}s"
    ...
    def test_search_complexity_performance(self, client):
        ...
        search_scenarios = [
            ...
            {
                "query": "python developer remote full-time senior 80000",
                "complexity": "complex",
            },
            {
                "query": "python developer remote full-time senior 80000-120000 san francisco",
        ...
        for scenario in search_scenarios:
            ...
            for _ in range(5):
                start_time = time.time()
                response = client.get(
                    f"/api/v1/jobs/search?q={scenario['query']}&limit=10"
                )
                end_time = time.time()
            ...
            if scenario["complexity"] in ["simple", "medium"]:
                assert (
                    avg_time < 2.0
                ), f"Search too slow for {scenario['complexity']}: {avg_time:.3f}s"
            else:
                assert (
                    avg_time < 3.0
                ), f"Search too slow for {scenario['complexity']}: {avg_time:.3f}s"
    ...
    def test_error_handling_performance(self, client):
        ...
        for scenario in error_scenarios:
            ...
            assert (
                avg_time < 1.0
            ), f"Error handling too slow for {scenario}: {avg_time:.3f}s"
...
class TestStressPerformance:
    ...
    def test_rapid_fire_requests(self, client):
        ...
        ), f"Average rapid response time too slow: {statistics.mean(response_times):.3f}s"
    ...
    def test_large_payload_handling(self, client):
        ...
        for query in large_queries:
            ...
            assert (
                response_time < 5.0
            ), f"Large payload handling too slow: {response_time:.3f}s"
