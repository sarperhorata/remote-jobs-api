...
async def get_ai_service(db: AsyncIOMotorDatabase = Depends(get_async_db)) -> AIJobMatchingService:
...
async def get_job_match_score(
    ...
    try:
        ...
        logger.error(f"Error calculating match score for job {job_id}, user {user_id}: {e}")
...
async def get_skills_demand_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to analyze skills demand: {str(e)}")
...
async def get_salary_insights(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get salary insights: {str(e)}")
