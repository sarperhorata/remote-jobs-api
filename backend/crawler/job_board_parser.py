...
class JobBoardParser:
    ...
    def __init__(self):
        ...
        self.parsers = {"linkedin": LinkedInParser(), "remotive": RemotiveParser()}
        logger.info(
            f"Initialized JobBoardParser with sources: {', '.join(self.parsers.keys())}"
        )
    ...
    def get_jobs(
        ...
        logger.info(
            f"Fetching jobs from sources: {sources} with keywords: '{keywords}', location: '{location}'"
        )
        ...
        with ThreadPoolExecutor(max_workers=len(sources)) as executor:
            future_to_source = {
                executor.submit(
                    self._fetch_jobs_from_source, source, keywords, location, limit
                ): source
    ...
    def _fetch_jobs_from_source(
        self, source: str, keywords: str, location: str, limit: int
    ) -> List[Dict[str, Any]]:
    ...
    def get_job_details(self, job_id: str, source: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting job details for job {job_id} from {source}: {e}"
            )
            return {}
    ...
    def search_jobs(
        ...
        return self.get_jobs(
            sources=sources, keywords=keyword, location=location, limit=limit
        )
    ...
    def get_job_categories(self, sources: List[str] = None) -> Dict[str, List[str]]:
        ...
        for source in sources:
            try:
                ...
                logger.info(
                    f"Retrieved {len(source_categories)} categories from {source}"
                )
            except Exception as e:
