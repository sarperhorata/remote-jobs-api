...
from backend.utils.email import (create_password_reset_token,
...
async def register_user(
    request: Request, user: UserCreate, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
    """Register a new user."""
...
async def google_callback(code: str, db: AsyncIOMotorDatabase = Depends(get_async_db)):
...
async def forgot_password(
    request: ForgotPasswordRequest, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def reset_password(
    request: ResetPasswordRequest, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def linkedin_callback(
    code: str, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def linkedin_fetch_cv(
    ...
    try:
        ...
        if not user or not user.get("linkedin_connected"):
            raise HTTPException(
                status_code=400, detail="LinkedIn account not connected"
            )
        ...
        if not linkedin_token:
            raise HTTPException(
                status_code=400, detail="LinkedIn access token not available"
            )
        ...
        if not cv_data:
            raise HTTPException(
                status_code=400, detail="Failed to fetch LinkedIn CV data"
            )
