...
class DatabaseIndexOptimizer:
    ...
    async def create_jobs_indexes(self):
        ...
        indexes = [
            ...
            IndexModel([("location", ASCENDING), ("remote", ASCENDING)], name="location_remote_compound"),
            ...
            IndexModel([("is_active", ASCENDING), ("posted_date", DESCENDING)], name="active_posted_compound"),
            ...
            IndexModel([("salary_min", ASCENDING), ("salary_max", DESCENDING)], name="salary_range_compound"),
            ...
            IndexModel([("external_id", ASCENDING)], name="external_id_asc", unique=True, sparse=True),
            
            # Application and view statistics
            IndexModel([("view_count", DESCENDING)], name="view_count_desc"),
            IndexModel([("application_count", DESCENDING)], name="application_count_desc"),
        ...
        try:
            ...
            for index in indexes:
                try:
                    await jobs_collection.create_index(index.document['key'], **{k: v for k, v in index.document.items() if k != 'key'})
                    index_name = index.document.get('name', 'unknown')
                    self.optimization_report['indexes_created'].append(f"jobs.{index_name}")
                    created_count += 1
    ...
    async def create_companies_indexes(self):
        ...
        indexes = [
            ...
            # External tracking
            IndexModel([("external_id", ASCENDING)], name="company_external_id", unique=True, sparse=True),
        ...
        try:
            ...
            self.optimization_report['indexes_created'].extend([f"companies.{name}" for name in result])
    ...
    async def create_users_indexes(self):
        ...
        indexes = [
            ...
            IndexModel([("username", ASCENDING)], name="username_asc", unique=True, sparse=True),
            ...
            IndexModel([("profile_visibility", ASCENDING)], name="profile_visibility_asc"),
        ...
        try:
            ...
            self.optimization_report['indexes_created'].extend([f"users.{name}" for name in result])
    ...
    async def create_applications_indexes(self):
        ...
        indexes = [
            ...
            IndexModel([("user_id", ASCENDING), ("job_id", ASCENDING)], name="user_job_compound", unique=True),
            ...
            IndexModel([("company_id", ASCENDING), ("status", ASCENDING)], name="company_status_compound"),
            
            # Application source and method
            IndexModel([("application_method", ASCENDING)], name="application_method_asc"),
            IndexModel([("source", ASCENDING)], name="app_source_asc"),
            
            # Status timeline
            IndexModel([("user_id", ASCENDING), ("applied_date", DESCENDING)], name="user_applied_timeline"),
            IndexModel([("job_id", ASCENDING), ("applied_date", DESCENDING)], name="job_applied_timeline"),
        ]
        ...
        try:
            ...
            self.optimization_report['indexes_created'].extend([f"applications.{name}" for name in result])
            ...
            self.optimization_report['errors'].append(f"Applications indexes failed: {e}")
    ...
    async def create_activity_logs_indexes(self):
        ...
        indexes = [
            ...
            IndexModel([("user_id", ASCENDING), ("timestamp", DESCENDING)], name="user_activity_timeline"),
            ...
            IndexModel([("timestamp", ASCENDING)], name="activity_ttl", expireAfterSeconds=2592000),
        ]
        
        try:
            result = await activity_collection.create_indexes(indexes)
            logger.info(f"✅ Created {len(result)} indexes for activity logs collection")
            self.optimization_report['indexes_created'].extend([f"activity_logs.{name}" for name in result])
            ...
            self.optimization_report['errors'].append(f"Activity logs indexes failed: {e}")
    ...
    async def cleanup_unused_indexes(self):
        ...
        try:
            collections = ['jobs', 'companies', 'users', 'applications', 'activity_logs']
            for collection_name in collections:
