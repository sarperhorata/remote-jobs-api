...
import jwt
import pytest
from utils.email import (ACCESS_TOKEN_EXPIRE_MINUTES, ALGORITHM, SECRET_KEY,
...
class TestUtilsEmail:
    ...
    def test_send_verification_email_success(self, mock_mailgun):
        ...
        assert result is True
        mock_mailgun.send_verification_email.assert_called_once_with(
    ...
    def test_send_password_reset_email_success(self, mock_mailgun):
        ...
        assert result is True
        mock_mailgun.send_password_reset_email.assert_called_once_with(
            "test@example.com", "reset123"
        )
