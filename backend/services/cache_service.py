...
class CacheService:
    ...
    def __init__(self, max_size: int = 100, ttl_hours: int = 24):
        ...
        logger.info(f"✅ Cache service initialized with max_size={max_size}, ttl={ttl_hours}h, popular_ttl={self.popular_ttl_hours}h")
    
    def _is_popular_keyword(self, keyword: str) -> bool:
        """Check if keyword is in top 10 popular searches."""
        popular_keywords = [
            "react", "python", "javascript", "java", "node", 
            "frontend", "backend", "fullstack", "devops", "data"
        ]
        keyword_lower = keyword.lower().strip()
        return any(pop_kw in keyword_lower or keyword_lower in pop_kw for pop_kw in popular_keywords)
    
    def _cleanup_expired(self, cache_dict: OrderedDict, expiry_dict: Dict[str, float]) -> None:
        """Remove expired entries from cache."""
        current_time = time.time()
        expired_keys = [
            key for key, expiry_time in expiry_dict.items() 
    ...
    async def set(self, key: str, value: Any, ttl_hours: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.debug(f"💾 Cached key: {key} (popular: {is_popular}, ttl: {ttl_hours}h)")
            return True
