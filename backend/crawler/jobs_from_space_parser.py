            ...
            for script in scripts:
                ...
                if script.string:
                    ...
                    data_matches = re.findall(
                        r"window\.[A-Za-z0-9_]+ = ({.+?});", script.string, re.DOTALL
                    )
                    if data_matches:
    ...
    def get_company_list(self):
        ...
        try:
            ...
            company_elements = soup.select(
                ".company-card, .company-listing, .company-item"
            )
            ...
            if not company_elements:
                ...
                company_elements = soup.find_all(
                    "div",
                    class_=lambda c: c
                    and ("company" in c.lower() or "employer" in c.lower()),
                )

            for company in company_elements:
                company_name = (
                    company.find("h3") or company.find("h2") or company.find("h4")
                )
                    ...
                    companies.append(
                        {
                            ...
                            "url": (
                                company_link["href"]
                                if company_link["href"].startswith("http")
                                else f"{self.base_url}{company_link['href']}"
                            ),
                        }
                    )
        ...
        try:
            for page in range(1, max_pages + 1):
                ...
                job_elements = soup.select(
                    ".job-card, .job-listing, .job-item, .list-item"
                )
                ...
                if not job_elements:
                    ...
                    job_elements = soup.find_all(
                        "div",
                        class_=lambda c: c
                        and ("job" in c.lower() or "listing" in c.lower()),
                    )
                ...
                for job in job_elements:
                    try:
                        ...
                        url = (
                            ...
                            else f"{self.base_url}{link_elem['href']}"
                        )
                        ...
                        if tags_elem:
                            tag_items = tags_elem.find_all(
                                "span"
                            ) or tags_elem.find_all("a")
                ...
                next_page = soup.find("a", string="Next") or soup.find(
                    "a", class_=lambda c: c and ("next" in c.lower())
                )
                if not next_page:
                    break
