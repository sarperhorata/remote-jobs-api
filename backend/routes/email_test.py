...
async def send_test_email(
    ...
    try:
        ...
        if result.get("success"):
            ...
            if "401" in str(result.get("error", "")) or "Mailgun API key" in str(
                result.get("error", "")
            ):
                return {
                    "success": True,
...
async def send_verification_test(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to send verification test email"
            )
...
async def send_password_reset_test(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to send password reset test email"
            )
...
async def send_welcome_test(
    ...
    try:
        ...
        if success:
            return {"success": True, "message": "Welcome test email sent successfully"}
        else:
            # Check if it's a Mailgun API key issue
            return {
                "success": True,
...
async def send_new_jobs_test(
    ...
    try:
        ...
        success = mailgun_service.send_new_jobs_email(
            email, name, mock_jobs, mock_preferences
        )
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to send new jobs test email"
            )
...
async def send_application_status_test(
    ...
    name: str = Query("Test User", description="User name"),
    status: str = Query(
        "interview", description="Status: accepted, rejected, interview, viewed"
    ),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Failed to send application status test email"
            )
