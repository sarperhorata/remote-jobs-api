...
class MonitoringSystem:
    ...
    def collect_application_metrics(self) -> ApplicationMetrics:
        ...
        try:
            ...
            if self.redis_client:
                ...
                active_connections = int(
                    self.redis_client.get("app:active_connections") or 0
                )
            else:
    ...
    def collect_database_metrics(self) -> DatabaseMetrics:
        ...
        try:
            ...
            connection_count = len(
                self.mongo_client.server_info()["connections"]["current"]
            )
    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        summary = {"system": {}, "application": {}, "database": {}, "alerts": []}
        ...
        if system_metrics:
            summary["system"] = {
                ...
                "avg_memory": statistics.mean(
                    [m.memory_percent for m in system_metrics]
                ),
                "max_memory": max([m.memory_percent for m in system_metrics]),
                "avg_disk": statistics.mean(
                    [m.disk_usage_percent for m in system_metrics]
                ),
        ...
        app_metrics = [
            m for m in self.metrics_history["application"] if m.timestamp > cutoff_time
        ]
        if app_metrics:
            summary["application"] = {
                ...
                "avg_response_time": statistics.mean(
                    [m.avg_response_time for m in app_metrics]
                ),
                "max_response_time": max([m.avg_response_time for m in app_metrics]),
                "avg_connections": statistics.mean(
                    [m.active_connections for m in app_metrics]
                ),
            }

        # Database metrics summary
        db_metrics = [
            m for m in self.metrics_history["database"] if m.timestamp > cutoff_time
        ]
        if db_metrics:
            summary["database"] = {
                "avg_connections": statistics.mean(
                    [m.connection_count for m in db_metrics]
                ),
