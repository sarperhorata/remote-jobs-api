...
class TestSchedulerService:
    ...
    def scheduler_service(self, mock_scheduler):
        ...
        with patch('services.scheduler_service.AsyncIOScheduler', return_value=mock_scheduler):
    ...
    def mock_external_services(self):
        ...
        with patch('services.scheduler_service.ExternalJobAPIManager') as mock_api_manager, \
             patch('services.scheduler_service.ServiceNotifier') as mock_notifier, \
             patch('services.scheduler_service.DistillCrawler') as mock_crawler:
            
            mock_api_manager.return_value.fetch_all_jobs.return_value = []
            mock_api_manager.return_value.save_jobs_to_database.return_value = {"api1": 10, "api2": 5}
            
            mock_notifier.return_value._send_message = Mock()
            
            mock_crawler.return_value.load_companies_data = Mock()
            mock_crawler.return_value.companies_data = [{"name": "Company1"}, {"name": "Company2"}]
            mock_crawler.return_value.crawl_all_companies = AsyncMock(return_value={"total_jobs": 15})
    ...
    @pytest.mark.asyncio
    async def test_start_scheduler_success(self, scheduler_service, mock_external_services):
    ...
    async def test_setup_jobs(self, scheduler_service):
        ...
        health_check_calls = [call for call in add_job_calls if 'health_check' in str(call)]
        assert len(health_check_calls) > 0
        
        # Check for external API crawler job
        external_api_calls = [call for call in add_job_calls if 'external_api_crawler' in str(call)]
        assert len(external_api_calls) > 0
        
        # Check for distill crawler job
        distill_calls = [call for call in add_job_calls if 'distill_crawler' in str(call)]
        assert len(distill_calls) > 0
        
        # Check for database cleanup job
        cleanup_calls = [call for call in add_job_calls if 'database_cleanup' in str(call)]
    ...
    @pytest.mark.asyncio
    async def test_external_api_crawler_job(self, scheduler_service, mock_external_services):
        """External API crawler job testi"""
        await scheduler_service._external_api_crawler_job()
        
        # Verify that external services are called
        mock_external_services['api_manager'].return_value.fetch_all_jobs.assert_called_once()
        mock_external_services['api_manager'].return_value.save_jobs_to_database.assert_called_once()
    ...
    async def test_distill_crawler_job(self, scheduler_service, mock_external_services):
        ...
        mock_external_services['crawler'].return_value.load_companies_data.assert_called_once()
        mock_external_services['crawler'].return_value.crawl_all_companies.assert_called_once()
        mock_external_services['notifier'].return_value._send_message.assert_called()
