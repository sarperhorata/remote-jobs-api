...
from utils.db import (create_website, delete_website, get_website,
                      get_websites, update_website)

from models.models import SelectorBase, Website, WebsiteCreate, WebsiteType
...
async def get_all_websites(
    website_type: Optional[WebsiteType] = Query(
        None, description="Filter by website type"
    ),
...
async def remove_selector_from_website(website_id: int, selector_name: str):
    ...
    if len(selectors) == original_count:
        raise HTTPException(
            status_code=404, detail=f"Selector with name '{selector_name}' not found"
        )
    ...
    return {
        "message": f"Selector '{selector_name}' removed from website ID {website_id}"
    }
