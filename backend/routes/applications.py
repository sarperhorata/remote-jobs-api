    ...
    **Application Types:**
    ...
    try:
        ...
        existing = await service.get_user_application(
            user_id=current_user["_id"], job_id=application_data.job_id
        )
        ...
        logger.info(
            f"User {current_user['_id']} applied to job {application_data.job_id}"
        )
...
async def get_my_applications(
    ...
    status_filter: Optional[str] = Query(
        None,
        description="Filter by application status (applied, viewed, rejected, hired, withdrawn)",
    ),
):
    """
...
@router.get(
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {"example": ["job_123", "job_456", "job_789"]}
            },
        },
        401: {"description": "Authentication required"},
        500: {"description": "Server error"},
...
async def get_applied_job_ids(
    ...
    try:
        service = get_user_application_service()
        job_ids = await service.get_applied_job_ids(user_id=current_user["_id"])
        return job_ids
...
async def check_if_applied(
    job_id: str, current_user: dict = Depends(get_current_user_dependency)
):
    """
    ...
    try:
        ...
        application_data = {
            ...
            "company_response_date": (
                application.get("company_response_date").isoformat()
                if application.get("company_response_date")
                else None
            ),
...
async def get_application_stats(
    db=Depends(get_async_db), current_user: dict = Depends(get_current_user_dependency)
):
    """
    ...
    try:
        ...
        for app_status in statuses:
            count = await db.job_applications.count_documents(
                {"user_id": current_user_id, "status": app_status}
            )
        ...
        for app_type in types:
            count = await db.job_applications.count_documents(
                {"user_id": current_user_id, "application_type": app_type}
            )
        ...
        recent_applications = await db.job_applications.count_documents(
            {"user_id": current_user_id, "applied_at": {"$gte": week_ago}}
        )

        # Response rate (applications with company response)
        responded_applications = await db.job_applications.count_documents(
            {"user_id": current_user_id, "company_response": {"$ne": None}}
        )

        response_rate = (
            (responded_applications / total_applications * 100)
...
async def delete_application(
    application_id: str, current_user: dict = Depends(get_current_user_dependency)
):
