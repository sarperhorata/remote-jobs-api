...
class TestPerformanceAnalyticsService:
    ...
    async def test_track_api_call_error(self, analytics_service):
        ...
        analytics_service.db.performance_metrics.insert_one = AsyncMock(side_effect=Exception("DB error"))
    ...
    @pytest.mark.asyncio
    async def test_track_user_activity_success(self, analytics_service, sample_user_activity):
    ...
    async def test_track_user_activity_error(self, analytics_service):
        ...
        analytics_service.db.user_activities.insert_one = AsyncMock(side_effect=Exception("DB error"))
        
        # Should not raise exception
        await analytics_service.track_user_activity("user123", "search", {"query": "test"})
    ...
    async def test_track_error_success(self, analytics_service):
        ...
        await analytics_service.track_error("Database connection failed", "critical", "user123")
    ...
    async def test_get_api_performance_metrics(self, analytics_service):
        ...
        analytics_service.db.performance_metrics.aggregate = Mock(return_value=mock_cursor)
    ...
    async def test_get_system_health_metrics(self, analytics_service):
        ...
        analytics_service.db.performance_metrics.count_documents = AsyncMock(return_value=1000)
        analytics_service.db.user_activities.count_documents = AsyncMock(return_value=500)
    ...
    async def test_service_integration(self, analytics_service):
        ...
        await analytics_service.track_api_call("/api/v1/jobs", 150.5, 200)
        await analytics_service.track_user_activity("user123", "search", {"query": "python"})
