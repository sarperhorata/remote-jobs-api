...
from backend.database.db import (close_db_connections, get_async_db,
                                 init_database)
...
async def lifespan(app: FastAPI):
    ...
    if (
        not is_testing
        and os.getenv("DISABLE_TELEGRAM") != "true"
        and TELEGRAM_BOT_AVAILABLE
    ):
        telegram_bot = await get_managed_bot()

    if (
        not is_testing
        and os.getenv("DISABLE_SCHEDULER") != "true"
        and SCHEDULER_AVAILABLE
    ):
...
if os.getenv("ENVIRONMENT") == "production":
    ...
    os.getenv("ENVIRONMENT") == "development" or os.getenv("NODE_ENV") == "development"
):
...
from backend.middleware.rate_limiting import custom_rate_limit_handler, limiter
...
from backend.middleware.input_validation import \
    initialize_input_validation_middleware
...
if os.path.exists(admin_static_path):
    app.mount(
        "/admin/static", StaticFiles(directory=admin_static_path), name="admin_static"
    )
    ...
    try:
        ...
        return {
            ...
            "services": {
                ...
                "admin_panel": os.getenv("ADMIN_PANEL_ENABLED", "true").lower()
                == "true",
            },
            "timestamp": datetime.utcnow().isoformat(),
        }
...
async def get_job_statistics():
    ...
    try:
        ...
        recent_jobs = await jobs_col.count_documents({"created_at": {"$gte": week_ago}})
...
async def get_job_statistics_v1():
    ...
    try:
        ...
        active_companies = await companies_col.count_documents(
            {"is_active": {"$ne": False}}
        )
        ...
        unique_companies_result = await jobs_col.aggregate(
            unique_companies_pipeline
        ).to_list(length=None)
        unique_companies_count = (
            unique_companies_result[0]["total"] if unique_companies_result else 0
        )
        ...
        unique_locations_result = await jobs_col.aggregate(
            unique_locations_pipeline
        ).to_list(length=None)
        countries_count = (
            unique_locations_result[0]["total"] if unique_locations_result else 0
        )
        ...
        return {
            ...
            "companies_count": max(
                unique_companies_count, active_companies, 820
...
async def get_featured_jobs():
    ...
    try:
        ...
        featured_jobs = (
            await jobs_col.find({"is_active": True, "work_type": "remote"})
            .sort("created_at", -1)
            .limit(10)
            .to_list(length=10)
        )

        return {"jobs": featured_jobs, "total": len(featured_jobs)}
    except Exception as e:
...
async def get_featured_companies():
    ...
    try:
        ...
        return {"companies": featured_companies, "total": len(featured_companies)}
    except Exception as e:
...
async def get_search_suggestions(q: str = "", limit: int = 5):
    ...
    try:
        ...
        pipeline = [
            {"$match": {"title": {"$regex": q, "$options": "i"}, "is_active": True}},
        ...
        return {"suggestions": [s["_id"] for s in suggestions], "query": q}
    except Exception as e:
        ...
        result = await db.users.update_one(
            {"_id": ObjectId(current_user["id"])},
            {
                "$set": {
                    ...
                    "browser_notifications": settings.get(
                        "browser_notifications", True
                    ),
                    "telegram_notifications": settings.get(
...
async def update_user_profile(
    ...
    try:
        ...
        result = await db.users.update_one(
            {"_id": ObjectId(current_user["id"])}, {"$set": profile_data}
        )
...
async def save_job(
    ...
    try:
        ...
        # Add to saved jobs
        result = await db.users.update_one(
            {"_id": ObjectId(current_user["id"])}, {"$addToSet": {"saved_jobs": job_id}}
        )
...
async def unsave_job(
    ...
    try:
        result = await db.users.update_one(
            {"_id": ObjectId(current_user["id"])}, {"$pull": {"saved_jobs": job_id}}
        )
...
async def get_user_applications(
    ...
    try:
        ...
        applications = (
            await applications_col.find({"user_id": ObjectId(current_user["id"])})
            .sort("created_at", -1)
            .skip(skip)
            .limit(limit)
            .to_list(length=limit)
        ...
        result = await db.users.update_one(
            {"_id": ObjectId(current_user["id"])},
            {
                "$set": {
                    ...
                    "preferred_experience_levels": preferences.get(
                        "experience_levels", []
                    ),
...
async def get_dashboard_metrics():
    ...
    try:
        ...
        # Get recent activity
        week_ago = datetime.utcnow() - timedelta(days=7)
        recent_jobs = await db.jobs.count_documents({"created_at": {"$gte": week_ago}})
        ...
        return {
            "jobs": {"total": jobs_count, "active": active_jobs, "recent": recent_jobs},
            "companies": companies_count,
            "users": users_count,
            "applications": {"recent": recent_applications},
...
async def get_similar_jobs(job_id: str, limit: int = 5):
    ...
    try:
        ...
        return {"similar_jobs": similar_jobs, "total": len(similar_jobs)}
    except Exception as e:
...
async def get_locations_autocomplete(q: str = "", limit: int = 10):
    ...
    try:
        ...
        pipeline = [
            {"$match": {"location": {"$regex": q, "$options": "i"}, "is_active": True}},
            {"$group": {"_id": "$location"}},
            {"$sort": {"_id": 1}},
            {"$limit": limit},
        ]
...
async def get_skills_autocomplete(q: str = "", limit: int = 10):
    ...
    try:
        ...
        pipeline = [
            {"$match": {"skills": {"$regex": q, "$options": "i"}, "is_active": True}},
            {"$unwind": "$skills"},
...
RENDER_IPS = ["100.20.92.101", "44.225.181.72", "44.227.217.144"]
        ...
        if SCHEDULER_AVAILABLE:
            try:
                ...
                if scheduler:
                    ...
                    return {
                        "status": "success",
                        "message": "External API crawler completed",
                        "timestamp": datetime.utcnow().isoformat(),
                    }
            except Exception as scheduler_error:
                logger.warning(f"Scheduler not available: {scheduler_error}")
        ...
        if SCHEDULER_AVAILABLE:
            try:
                ...
                if scheduler:
                    ...
                    return {
                        "status": "success",
                        "message": "Distill crawler completed",
                        "timestamp": datetime.utcnow().isoformat(),
                    }
            except Exception as scheduler_error:
                logger.warning(f"Scheduler not available: {scheduler_error}")
        ...
        if SCHEDULER_AVAILABLE:
            try:
                ...
                if scheduler:
                    ...
                    return {
                        "status": "success",
                        "message": "Job statistics completed",
                        "timestamp": datetime.utcnow().isoformat(),
                    }
            except Exception as scheduler_error:
                logger.warning(f"Scheduler not available: {scheduler_error}")
