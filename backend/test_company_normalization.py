...
# Add paths
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "scripts", "archive"))
)
...
class CompanyNormalizationTester:
    ...
    def test_normalization(self):
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            ...
            print(
                f"    Variations ({len(variations)}): {variations[:5]}{'...' if len(variations) > 5 else ''}"
            )
            ...
            if expected:
                ...
                for exp in expected:
                    if any(
                        exp.lower() in var.lower() or var.lower() in exp.lower()
        ...
        if success_rate >= 80:
            ...
            print(
                f"⚠️  NEEDS IMPROVEMENT! Success rate is {success_rate:.1f}% (Target: 80%+)"
            )
    ...
    def test_specific_patterns(self):
        ...
        for test_input, expected in specific_tests:
            ...
            if expected:
                ...
                found = any(
                    expected.lower() in var.lower() or var.lower() in expected.lower()
                    for var in variations
                )
                status = "✅" if found else "❌"
                print(
                    f"{status} '{test_input}' → Expected: '{expected}' | Got: {variations[:3]}"
                )
            else:
                # Should be filtered out or minimal variations
                status = (
                    "✅" if len([v for v in variations if len(v) > 3]) <= 2 else "❌"
                )
                print(
                    f"{status} '{test_input}' → Should be filtered | Got: {variations[:3]}"
                )
