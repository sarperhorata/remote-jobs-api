...
class RateLimiter:
    ...
    def can_make_request(self) -> bool:
        now = time.time()
        # Remove old requests
        self.requests = [req_time for req_time in self.requests if now - req_time < self.time_period_seconds]
...
class ServiceNotifier:
    ...
    def _send_fallback_crawler_message(self, crawler_data: Dict[str, Any]):
        ...
        if 'companies_processed' in crawler_data:
            message += f"<b>Companies Processed:</b> {crawler_data['companies_processed']}\n"
...
class APIErrorHandler:
    ...
    def handle_api_error(self, api_name: str, endpoint: str, status_code: int, error_message: str):
        ...
        if status_code == 429 or any(pattern in error_lower for pattern in self.quota_patterns):
            self.mark_quota_exceeded(api_name, error_message)
            return
        
        # Check for 404 errors (endpoint not found)
        if status_code == 404:
            self.disable_endpoint(api_name, endpoint, f"Endpoint not found (404): {error_message}")
            return
        
        # Check for 403 errors (access forbidden)
        if status_code == 403:
            self.disable_endpoint(api_name, endpoint, f"Access forbidden (403): {error_message}")
            return
...
⏳ <b>Action:</b> Will retry later""")
    ...
    def disable_endpoint(self, api_name: str, endpoint: str, reason: str):
        ...
        if key not in self.disabled_endpoints:
            ...
            self.notifier._send_message(f"""🚫 <b>{api_name} - ENDPOINT PERMANENTLY DISABLED</b>
...
📋 <b>Total disabled endpoints:</b> {len(self.disabled_endpoints)}""")
            ...
            logger.warning(f"🚫 Permanently disabled endpoint: {api_name}/{endpoint} - {reason}")
...
📊 <b>Total APIs with quota issues:</b> {len(self.quota_exceeded)}""")
            ...
            logger.warning(f"📛 API quota exceeded: {api_name} - will resume next month")
    ...
    def get_disabled_endpoints_summary(self) -> Dict[str, Any]:
        ...
        for key, endpoint_data in self.disabled_endpoints.items():
            disabled_list.append({
                ...
                'reason': endpoint_data.get('reason', '')[:100] + ('...' if len(endpoint_data.get('reason', '')) > 100 else ''),
                'disabled_at': endpoint_data.get('disabled_at')
            })
        
        return {
            'count': len(disabled_list),
            'endpoints': disabled_list
        }
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_apis_affected': len(self.disabled_endpoints) + len(quota_exceeded_this_month),
