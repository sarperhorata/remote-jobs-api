...
class TestJobMatchingService:
    ...
    async def test_get_job_recommendations_empty_jobs(self, service, mock_db):
        ...
        mock_db.user_skills.find.return_value.to_list.return_value = AsyncMock(return_value=[])
        mock_db.user_experience.find.return_value.to_list.return_value = AsyncMock(return_value=[])
        mock_db.user_preferences.find_one.return_value = AsyncMock(return_value={})
        mock_db.applications.find.return_value.limit.return_value.to_list.return_value = AsyncMock(return_value=[])
        
        # Mock empty jobs
        mock_db.jobs.find.return_value.limit.return_value.to_list.return_value = AsyncMock(return_value=[])
    ...
    async def test_match_analytics_empty_data(self, service, mock_db):
        ...
        mock_db.recommendation_logs.find.return_value.sort.return_value.limit.return_value.to_list.return_value = AsyncMock(return_value=[])
        mock_db.applications.find.return_value.sort.return_value.limit.return_value.to_list.return_value = AsyncMock(return_value=[])
