...
class AutoApplicationService:
    ...
    async def submit_automated_application(
        ...
        try:
            ...
            if application_flow == "external_redirect":
                ...
                return await self._handle_embedded_form(
                    job_data, user_profile, preferences
                )
            elif application_flow == "multi_step":
                return await self._handle_multi_step_application(
                    job_data, user_profile, preferences
                )
            else:
                return {
            ...
            if forms:
                for form in forms:
                    ...
                    if any(
                        ...
                        for keyword in ["name", "email", "resume", "cv", "experience"]
                    ):
    ...
    async def _handle_external_redirect(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"External redirect failed: {str(e)}"}
    ...
    async def _handle_embedded_form(
        ...
        try:
            ...
            filled_fields = await self._auto_fill_form(
                form_elements, user_profile, job_data
            )
            ...
            if submission_result["success"]:
                return {
                    ...
                    "details": {
                        ...
                        "submission_confirmation": submission_result.get(
                            "confirmation"
                        ),
                    },
        ...
        try:
            ...
            if element_id:
                label = self.driver.find_element(
                    By.XPATH, f"//label[@for='{element_id}']"
                )
                if label:
    ...
    async def _auto_fill_form(
        ...
        for elem_info in form_elements:
            try:
                ...
                logger.warning(
                    f"Failed to fill field {elem_info.get('name', 'unknown')}: {str(e)}"
                )
                continue
    ...
    def _get_field_value(
        ...
        if any(
            keyword in label
            for keyword in ["name", "full name", "first name", "last name"]
        ):
            if "first" in label:
                return (
                    user_profile.get("name", "").split()[0]
        ...
        # Location fields
        if any(
            keyword in label for keyword in ["location", "city", "address", "where"]
        ):
        ...
        if any(
            keyword in label
            for keyword in ["cover letter", "why", "interest", "motivation"]
        ):
    ...
    def _generate_cover_letter_excerpt(
        self, user_profile: Dict[str, Any], job_data: Dict[str, Any]
    ) -> str:
        """
        ...
        if skills:
            excerpt += (
                f"With {experience} years of experience in {', '.join(skills[:3])}, "
            )
    ...
    def _detect_current_step(self) -> str:
        ...
        if any(
            keyword in page_text for keyword in ["personal", "contact", "name", "email"]
        ):
            ...
            for keyword in ["questions", "questionnaire", "additional"]
        ):
            return "questions"
        elif any(keyword in page_text for keyword in ["review", "confirm", "summary"]):
            return "review"
        elif any(
            keyword in page_text for keyword in ["complete", "thank you", "submitted"]
        ):
            return "complete"
        else:
            return "unknown"
    ...
    async def _fill_cover_letter_step(
        self, user_profile: Dict[str, Any], job_data: Dict[str, Any]
    ):
    ...
    async def _fill_questions_step(
        self, user_profile: Dict[str, Any], job_data: Dict[str, Any]
    ):
