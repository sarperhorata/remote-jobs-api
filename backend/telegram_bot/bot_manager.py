    ...
    def acquire_lock(self) -> bool:
        ...
        try:
            ...
            self.lock_fd = os.open(
                str(self.lock_path), os.O_CREAT | os.O_WRONLY | os.O_TRUNC
            )
    ...
    async def start_bot(self):
        ...
        if os.getenv("ENVIRONMENT") == "production":
            ...
            try:
                ...
                if bot_processes:
                    logger.warning(
                        f"⚠️  Found other potential bot processes: {bot_processes}. Skipping bot start to avoid conflicts."
                    )
                    return None
            except ImportError:
        ...
        if existing_pid:
            logger.warning(
                f"⚠️  Another Telegram bot instance is running (PID: {existing_pid})"
            )
            return None
        ...
        if not self.acquire_lock():
            logger.warning(
                "⚠️  Could not acquire lock, another instance might be starting"
            )
            return None
