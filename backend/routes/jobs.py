...
from backend.schemas.job import (ApplicationCreate, Job, JobCreate,
...
async def create_job(job: JobCreate, db: AsyncIOMotorDatabase = Depends(get_async_db)):
    """Create a new job."""
    ...
    if TELEGRAM_ENABLED:
        try:
            ...
            if telegram_bot_manager and telegram_bot_manager.bot:
                ...
                await telegram_bot_manager.bot.send_new_job_notification(
                    notification_data
                )
                logger.info(
                    f"✅ Telegram notification sent for new job: {job_dict.get('title', 'Unknown')}"
                )
...
async def read_jobs(
    ...
    skip: int = 0,
    limit: int = 50,
    db: AsyncIOMotorDatabase = Depends(get_async_db),  # Reduced from 100
):
...
async def search_jobs(
    ...
    limit: int = Query(
        20, ge=1, le=50, description="Number of results per page (optimized)"
    ),  # Reduced from 5000
    ...
    exact_title: Optional[str] = Query(
        None, description="Exact job title to search for"
    ),
    ...
    try:
        ...
        # Search text query - only apply if query is not empty and no exact title
            ...
            if q.lower() == "product manager":
                ...
                title_queries.insert(
                    0, {"title": {"$regex": "^Product Manager", "$options": "i"}}
                )
                title_queries.insert(
                    1, {"title": {"$regex": "Product Manager$", "$options": "i"}}
                )
                title_queries.insert(
                    2, {"title": {"$regex": "Product Manager\\b", "$options": "i"}}
                )
                ...
                query["$and"] = query.get("$and", []) + [
                    {
                        "title": {
                            "$not": {
                                "$regex": "Engineer|Developer|Programmer|Coder",
                                "$options": "i",
                            }
                        }
                    }
        ...
        if work_type:
            ...
            if work_type_lower == "remote":
                ...
                query["$and"] = query.get("$and", []) + [
                    ...
                    {
                        "remote_type": {
                            "$not": {"$regex": "remote|hybrid", "$options": "i"}
                        }
                    },
                    {
                        "work_type": {
        ...
        if job_type:
            ...
            query["$or"] = query.get("$or", []) + [
                {"job_type": {"$regex": pattern, "$options": "i"}}
        ...
        if country:
            ...
            if not location:  # Only apply if location filter is not already set
                ...
                query["location"] = {
                    "$regex": f"{location}.*{country}|{country}.*{location}",
                    "$options": "i",
                }
            ...
            try:
                ...
                if salary_range.endswith("+"):
                    ...
                    salary_or = [
                        ...
                        {
                            "salary_range": {
                                "$regex": f"{min_val//1000}k.*{max_val//1000}k|{max_val//1000}k.*{min_val//1000}k",
                                "$options": "i",
                            }
                        },
                        {
        ...
        total = (
            await db.jobs.count_documents(query)
            if query
            else await db.jobs.count_documents({})
        )
...
async def get_job_statistics(db: AsyncIOMotorDatabase = Depends(get_async_db)):
    ...
    try:
        ...
        for item in jobs_by_position:
            if item["_id"]:  # Filter out null/empty titles
                ...
                if any(
                    ...
                    for keyword in ["manager", "director", "lead", "head"]
                ):
                    category = "Management"
                elif any(
                    keyword in title_lower
                    ...
                    keyword in title_lower
                    for keyword in ["marketing", "sales", "business"]
                ):
                    category = "Marketing"
                elif any(
                    keyword in title_lower
                    for keyword in ["data", "analyst", "scientist"]
                ):
                    category = "Data"
                elif any(
                    keyword in title_lower
                    for keyword in ["devops", "infrastructure", "cloud"]
                ):
                    category = "Infrastructure"

                positions.append(
                    {"title": item["_id"], "count": item["count"], "category": category}
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Statistics not available"
        )
...
async def get_popular_job_titles(
    ...
    try:
        ...
        if cached_result:
            logger.info(
                f"✅ Cache hit for popular job titles: {len(cached_result)} results"
            )
        ...
        for item in job_titles:
            ...
            if title and title.strip():
                ...
                if (
                    normalized not in parsed_titles
                    or parsed_titles[normalized]["count"] < item["count"]
                ):
        ...
        sorted_results = sorted(
            parsed_titles.values(), key=lambda x: x["count"], reverse=True
        )
...
async def search_job_titles(
    ...
    try:
        ...
        if cached_result:
            logger.info(
                f"✅ Cache hit for job titles search: {q} -> {len(cached_result.get('suggestions', []))} results"
            )
            return cached_result
        ...
        search_patterns = [
            ...
            {
                "title": {"$regex": f"\\b{safe_q}", "$options": "i"}
            },  # Word boundary match
        ...
        for item in job_titles:
            ...
            if title and title.strip():
                ...
                if score >= 25:
                    ...
                    if (
                        normalized not in parsed_titles
                        or parsed_titles[normalized]["score"] < score
                    ):
                        parsed_titles[normalized] = {
                            "title": parsed.parsed_title,
                            "count": item["count"],
        ...
        sorted_results = sorted(
            parsed_titles.values(), key=lambda x: x["score"], reverse=True
        )
        final_results = []
        ...
        logger.info(
            f"💾 Cached job titles search result: {q} -> {len(final_results)} results"
        )
...
async def quick_search_count(
    ...
    try:
        ...
        if cached_result:
            logger.info(
                f"✅ Cache hit for quick search: {q} -> {cached_result['count']} jobs"
            )
            return cached_result
...
async def search_locations(
    ...
    try:
        ...
        if cached_result:
            logger.info(
                f"✅ Cache hit for locations search: {q} -> {len(cached_result.get('suggestions', []))} results"
            )
            return cached_result
        ...
        # Prepare result with cache timestamp
        result = {"suggestions": locations, "cached_at": datetime.utcnow().isoformat()}

        # Cache the result
        await cache_service.set(cache_key, result)

        logger.info(
            f"💾 Cached locations search result: {q} -> {len(locations)} results"
        )
        ...
        return {"suggestions": [], "cached_at": datetime.utcnow().isoformat()}
...
async def search_skills(
    ...
    try:
        ...
        filtered_skills = [
            skill for skill in common_skills if q.lower() in skill["name"].lower()
        ]
...
async def get_job_recommendations(
    ...
    try:
        ...
        cursor = (
            db.jobs.find({"is_active": {"$ne": False}})
            .sort("created_at", -1)
            .limit(limit)
        )
        ...
        raise HTTPException(
            ...
            detail="Recommendations not available",
        )
...
async def get_similar_jobs(
    ...
    try:
        ...
        job = await jobs_col.find_one(
            {"_id": ObjectId(job_id), "is_archived": {"$ne": True}}
        )
        if not job:
            raise HTTPException(
...
async def get_archived_jobs_admin(
    ...
    try:
        jobs_col = db["jobs"]
        archived_jobs = (
            await jobs_col.find({"is_archived": True})
            .skip(skip)
            .limit(limit)
            .to_list(length=limit)
        )
...
async def restore_job_admin(
    ...
    try:
        jobs_col = db["jobs"]
        success = await jobs_col.update_one(
            {"_id": ObjectId(job_id)}, {"$set": {"is_archived": False}}
        )
        if not success.modified_count:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Job not found or already restored",
            )
...
async def archive_old_jobs_endpoint(
    ...
    try:
        ...
        await jobs_col.update_many(
            {"is_archived": True}, {"$set": {"is_archived": False}}
        )
...
async def trigger_job_crawling(current_user: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        return {"message": "Job crawling completed", "result": result}
...
async def trigger_full_job_aggregation(current_user: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        return {"message": "Full job aggregation completed", "result": result}
...
async def get_data_sources_status(
    ...
    try:
        ...
        recent_jobs = await jobs_col.count_documents(
            {"created_at": {"$gte": yesterday}, "is_active": True}
        )

        # Remote job statistics
        remote_jobs = await jobs_col.count_documents(
            {"is_active": True, "remote_type": "remote"}
        )

        hybrid_jobs = await jobs_col.count_documents(
            {"is_active": True, "remote_type": "hybrid"}
        )
            ...
            },
            {
                "$group": {
                    ...
                    "jobs_with_description": {
                        "$sum": {"$cond": ["$has_description", 1, 0]}
                    },
                    "jobs_with_requirements": {
                        "$sum": {"$cond": ["$has_requirements", 1, 0]}
                    },
            ...
            quality_metrics = {
                ...
                "data_completeness": {
                    "salary_percentage": round(
                        (stats["jobs_with_salary"] / total) * 100, 2
                    ),
                    "description_percentage": round(
                        (stats["jobs_with_description"] / total) * 100, 2
                    ),
                    "requirements_percentage": round(
                        (stats["jobs_with_requirements"] / total) * 100, 2
                    ),
                    "skills_percentage": round(
                        (stats["jobs_with_skills"] / total) * 100, 2
                    ),
                    "company_percentage": round(
                        (stats["jobs_with_company"] / total) * 100, 2
                    ),
                },
                "avg_description_length": round(stats.get("avg_description_length", 0)),
                "overall_quality_score": round(
                    (
                        stats["jobs_with_salary"]
                        + stats["jobs_with_description"]
                        + stats["jobs_with_requirements"]
                        + stats["jobs_with_skills"]
                        + stats["jobs_with_company"]
                    )
                    / (total * 5)
                    * 100,
                    2,
...
async def cleanup_inactive_jobs(
    ...
    try:
        ...
        result = await jobs_col.delete_many(
            {"is_active": False, "last_updated": {"$lt": cutoff_date}}
        )
...
async def update_job_skills(
    ...
    try:
        ...
        while True:
            jobs = (
                await jobs_col.find({"is_active": True})
                .skip(skip)
                .limit(batch_size)
            ...
            for job in jobs:
                ...
                await jobs_col.update_one(
                    {"_id": job["_id"]}, {"$set": {"skills": new_skills}}
                )
                updated_count += 1

            skip += batch_size

        return {"message": f"Skills update completed", "updated_jobs": updated_count}
...
async def bookmark_job(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Bookmark failed"
        )
...
async def submit_scraped_form_application(
    ...
    try:
        ...
        if not scraped_form:
            raise HTTPException(
                status_code=404, detail="No scraped form found for this job"
            )
...
async def submit_automated_application(
    ...
    try:
        ...
        if not application_url:
            raise HTTPException(
                status_code=400, detail="No application URL found for this job"
            )
        ...
        logger.error(
            f"Error submitting automated application for job {job_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def track_job_interaction(
    job_id: str, tracking_data: Dict[str, Any] = Body(...), db=Depends(get_async_db)
):
    """
...
async def get_job_application_analytics(
    job_id: str, current_user: Any = Depends(get_current_user), db=Depends(get_async_db)
):
    """
    ...
    try:
        ...
        interactions = (
            await db["job_interactions"].find({"job_id": job_id}).to_list(None)
        )

        analytics = {
            "total_applications": len(applications),
            "application_methods": {
                "external_redirect": len(
                    [
                        a
                        for a in applications
                        if a.get("application_method") == "external"
                    ]
                ),
                "scraped_form": len(
                    [
                        a
                        for a in applications
                        if a.get("application_method") == "scraped_form"
                    ]
                ),
                "automated": len(
                    [
                        a
                        for a in applications
                        if a.get("application_method") == "automated"
                    ]
                ),
            },
            "total_views": len([i for i in interactions if i.get("action") == "view"]),
            "total_clicks": len(
                [i for i in interactions if i.get("action") == "external_redirect"]
            ),
