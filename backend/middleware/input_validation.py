...
class InputValidator:
    ...
    def sanitize_string(
        self, data: str, max_length: int = 1000, allow_html: bool = False
    ) -> str:
    ...
    def validate_email(self, email: str) -> str:
        ...
        if len(email) > 254:  # RFC 5321 limit
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Email address too long"
            )
    ...
    def _sanitize_json_recursive(self, data: Any) -> Any:
        ...
        if isinstance(data, str):
            ...
            return {
                key: self._sanitize_json_recursive(value) for key, value in data.items()
            }
        elif isinstance(data, list):
    ...
    def validate_search_query(self, query: str) -> str:
        ...
        if len(query) > 200:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Search query too long"
            )
...
class InputValidationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        self.validation_rules = {
            ...
            "/api/v1/auth/login": {"username": "email"},
            "/api/v1/jobs/search": {"q": "search"},
            "/api/v1/profile": {"email": "email", "phone": "phone", "website": "url"},
        }
