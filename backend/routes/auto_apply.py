...
async def analyze_job_form(
    request: FormAnalysisRequest, current_user: User = Depends(get_current_user)
):
    """Analyze a job application form to check if auto-apply is possible"""
    try:
        analysis = await auto_apply_service.analyze_job_application_form(
            str(request.job_url)
        )
...
async def auto_apply_to_job(
    request: AutoApplyRequest, current_user: User = Depends(get_current_user)
):
    """Automatically apply to a job using AI-generated responses"""
    try:
        # First, analyze the form
        form_analysis = await auto_apply_service.analyze_job_application_form(
            str(request.job_url)
        )
        ...
        field_responses = await auto_apply_service.generate_field_responses(
            user_profile, form_fields
        )
...
def calculate_profile_completeness(user_profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "overall_percentage": int(
            (
                (required_complete + optional_complete)
