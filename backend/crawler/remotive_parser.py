        ...
        try:
            ...
            for job in jobs:
                if not job.get("title") or not job.get("company_name"):
                    logger.warning(
                        f"Skipping job without required fields: {job.get('id')}"
                    )
                    continue
        ...
        try:
            ...
            for job_element in job_elements[:limit]:
                try:
                    ...
                    job_url = (
                        f"{self.base_url}{link_element['href']}"
                        if link_element["href"].startswith("/")
    ...
    def get_job_details(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            job_details = {
                ...
                "apply_url": (
                    apply_url_element["href"]
                    if apply_url_element and "href" in apply_url_element.attrs
                    else None
                ),
                "source": "remotive",
                "url": url,
