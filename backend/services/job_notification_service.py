...
class JobNotificationService:
    ...
    async def process_new_jobs_for_notifications(self, new_jobs: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for job in new_jobs:
                ...
                for user_id in matching_users:
                    ...
                    if await self._should_send_notification(user_id, job):
                        # Create notification
                        notification_created = await self._create_job_notification(user_id, job)
            ...
            logger.info(f"Created {total_notifications} notifications for {len(matched_users)} users")
    ...
    async def _job_matches_user_preferences(self, job: Dict[str, Any], user: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            if preferred_titles:
                ...
                title_match = any(title.lower() in job_title for title in preferred_titles)
                if not title_match:
                    return False
            ...
            if preferred_locations:
                ...
                location_match = any(loc.lower() in job_location for loc in preferred_locations)
                if not location_match and job.get("remote_type", "").lower() not in ["remote", "hybrid"]:
                    return False
            ...
            if preferred_companies:
                ...
                company_match = any(company.lower() in job_company for company in preferred_companies)
            ...
            if preferred_skills:
                ...
                skills_match = any(skill.lower() in job_description for skill in preferred_skills)
                if not skills_match:
                    return False
            ...
            work_type_match = any(work_type.lower() in job_work_type for work_type in preferred_work_types)
    ...
    async def _should_send_notification(self, user_id: str, job: Dict[str, Any]) -> bool:
        """
    ...
    async def _create_job_notification(self, user_id: str, job: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if result.inserted_id:
                logger.info(f"Created job notification for user {user_id}, job {job['_id']}")
                return True
            else:
    ...
    async def _send_desktop_notification(self, user_id: str, job: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Would send desktop notification to user {user_id} for job {job['_id']}")
    ...
    async def get_user_notification_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            total_notifications = await notifications_col.count_documents({"user_id": user_id})
