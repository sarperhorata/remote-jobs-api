...
from backend.models.models import (UserNotification, UserNotificationCreate,
...
async def get_user_notifications(
    ...
    try:
        ...
        cursor = (
            notifications_col.find(filter_query)
            .sort("created_at", -1)
            .skip(offset)
            .limit(limit)
        )
...
async def get_unread_count(
    ...
    try:
        ...
        count = await notifications_col.count_documents(
            {"user_id": user_id, "is_read": False, "is_active": True}
        )
...
async def create_notification(
    ...
    try:
        ...
        created_notification = await notifications_col.find_one(
...
async def delete_notification(
    ...
    try:
        ...
        # Soft delete
        await notifications_col.update_one(
            {"_id": ObjectId(notification_id)}, {"$set": {"is_active": False}}
        )
