...
async def estimate_salary(request: SalaryEstimationRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Maaş tahmini sırasında bir hata oluştu."
        )
...
async def extract_salary_from_text(text: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Maaş bilgisi çıkarılırken bir hata oluştu."
        )
...
async def get_similar_jobs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Benzer işler aranırken bir hata oluştu."
        )
...
async def process_job_salary(job_data: Dict):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="İş verisi işlenirken bir hata oluştu."
        )
...
async def get_salary_statistics():
    ...
    try:
        ...
        estimated_salary_jobs = await db.jobs.count_documents({"is_estimated": True})
        ...
        return {
            ...
            "salary_coverage_percentage": (
                round((jobs_with_salary / total_jobs * 100), 2) if total_jobs > 0 else 0
            ),
        ...
        raise HTTPException(
            status_code=500, detail="İstatistikler alınırken bir hata oluştu."
        )
