...
from models.models import ChangeLog, Job, Monitor, Website
...
class MonitorManager:
    ...
    def _start_monitor(self, monitor_id: int):
        ...
        """
        if (
            monitor_id in self.monitor_tasks
            and not self.monitor_tasks[monitor_id].done()
        ):
            ...
            try:
                ...
                if monitor.last_check:
                    ...
                    if next_check > now:
                        ...
                        logger.debug(
                            f"Monitor {monitor.name} waiting for {wait_seconds} seconds"
                        )
    ...
    async def _check_monitor(self, monitor: Monitor):
        ...
        try:
            ...
            if not website:
                logger.error(
                    f"Website {monitor.website_id} not found for monitor {monitor.name}"
                )
                return
    ...
    async def _compare_and_update_jobs(
        self, monitor: Monitor, new_jobs: List[Dict[str, Any]]
    ):
        """
    ...
    async def _mark_job_removed(self, monitor: Monitor, job: Job):
        ...
        change_log = {
            ...
            "job_id": job.id,
            "change_type": "removed",
            "old_data": {"title": job.title, "company": job.company, "url": job.url},
            "is_notified": False,
        }
    ...
    async def _send_notification(
        self, monitor: Monitor, change_type: str, job_data: Dict[str, Any]
    ):
        """
        ...
        for notification in notifications:
            await self.notification_manager.send_notification(
                notification, change_type, job_data
            )
