...
class TestFakeJobDetectionRoutes:
    ...
    def test_analyze_single_job_success(
        self, mock_analyze, client: TestClient, admin_headers
    ):
    ...
    def test_batch_analyze_jobs_success(
        self, mock_batch_analyze, client: TestClient, admin_headers
    ):
        ...
        response = client.post(
            ...
            json={"job_ids": ["job_1", "job_2"], "batch_size": 10},
            headers=admin_headers,
        )
    ...
    def test_fake_job_detection_admin_only(self, client: TestClient):
        ...
        for endpoint, method in endpoints:
            ...
            assert response.status_code in [
                ...
                404,
                422,
            ]  # Unauthorized, Forbidden, Not Found, or validation error

    @patch("services.fake_job_detector.FakeJobDetector.analyze_job")
    def test_analyze_job_error_handling(
        self, mock_analyze, client: TestClient, admin_headers
    ):
    ...
    @patch("services.fake_job_detector.FakeJobDetector.batch_analyze_jobs")
    def test_batch_analyze_large_batch(
        self, mock_batch_analyze, client: TestClient, admin_headers
    ):
        ...
        response = client.post(
            "/api/v1/fake-job-detection/batch-analyze",
            json={"job_ids": large_job_list, "batch_size": 50},
            headers=admin_headers,
        )
