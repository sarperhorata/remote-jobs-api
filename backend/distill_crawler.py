...
class DistillCrawler:
    ...
    def load_companies_data(self):
        ...
        try:
            ...
            distill_file_path = os.path.join(
                project_root, "distill-export", "Distill export - 01-18_2025-05-25.json"
            )
            ...
            logger.info(
                f"✅ Loaded {len(self.companies_data)} companies from distill export"
            )
            ...
            for company in self.companies_data:
                ...
                if uri not in unique_companies:
                    ...
                    logger.warning(
                        f"🔄 Skipping duplicate: {company.get('name')} - {uri}"
                    )

            self.companies_data = list(unique_companies.values())
            logger.info(
                f"✅ After removing duplicates: {len(self.companies_data)} unique companies"
            )
    ...
    async def crawl_company(self, company_data: Dict) -> List[JobListing]:
        ...
        try:
            ...
            logger.warning(
                f"🌐 Network error for {company_name} ({uri}): {str(e)} - skipping"
            )
            return []  # Return empty instead of raising exception

        except Exception as e:
            logger.warning(
                f"⚠️ Error crawling {company_name} ({uri}): {str(e)} - skipping"
            )
            return []  # Return empty instead of raising exception
    ...
    async def _crawl_job_platform(
        self, company_name: str, uri: str, config: Dict
    ) -> List[JobListing]:
                        ...
                        for include in includes:
                            ...
                            try:
                                if selector_type == "css":
                                    ...
                                    logger.debug(
                                        f"✅ CSS selector '{expr}' found {len(container_elements)} container elements"
                                    )
                                    ...
                                    for container in container_elements:
                                        ...
                                        individual_jobs = (
                                            self._find_individual_jobs_in_container(
                                                container
                                            )
                                        )
                                        if individual_jobs:
                                            ...
                                            logger.debug(
                                                f"✅ Found {len(individual_jobs)} individual jobs in container"
                                            )
                                        else:
                                    ...
                                    if css_selector:
                                        try:
                                            ...
                                            logger.debug(
                                                f"✅ XPath->CSS '{css_selector}' found {len(job_elements)} elements"
                                            )
                                        except Exception as css_error:
                                            logger.debug(
                                                f"❌ CSS selector '{css_selector}' failed: {css_error}"
                                            )
                                            job_elements = []
                                    else:
                                        # Fallback to generic selectors when XPath conversion fails
                                        logger.debug(
                                            f"⚠️ XPath conversion failed for '{expr}', using fallback"
                                        )
                                        try:
                                            fallback_selector = (
                                                self._get_fallback_selectors()
                                            )
                                            job_elements = soup.select(
                                                fallback_selector
                                            )
                                            logger.debug(
                                                f"✅ Fallback selectors found {len(job_elements)} elements"
                                            )
                                        except Exception as fallback_error:
                                            logger.debug(
                                                f"❌ Fallback selectors also failed: {fallback_error}"
                                            )
                                            job_elements = []
                                ...
                                logger.debug(
                                    f"❌ Selector error for {company_name}: {str(e)} - skipping"
                                )
                                # Try fallback approach
                                try:
                                    fallback_elements = soup.select(
                                        self._get_fallback_selectors()
                                    )
                                    ...
                                    logger.debug(
                                        f"❌ Fallback also failed: {fallback_error} - skipping"
                                    )
        ...
        try:
            ...
            for pattern in problematic_patterns:
                if re.search(pattern, xpath, re.IGNORECASE):
                    logger.debug(
                        f"Complex XPath pattern detected: {pattern} in {xpath}"
                    )
                    return None
            ...
            if any(char in css for char in ["(", ")", "[", "]"]) and not re.match(
                r'^[\w\s\-\[\]="\.:>#\+~]+$', css
            ):
    ...
    def _extract_lever_jobs(
        self, company_name: str, uri: str, soup: BeautifulSoup
    ) -> List[JobListing]:
        ...
        job_elements = soup.select(
            '.posting, .postings-group .posting, [data-qa="posting"]'
        )
        ...
        for element in job_elements:
            try:
                ...
                location_elem = element.select_one(
                    '.posting-categories .location, .location, [data-qa="posting-location"]'
                )
                location = (
                    location_elem.get_text(strip=True) if location_elem else "Remote"
                )
    ...
    def _extract_smartrecruiters_jobs(
        self, company_name: str, uri: str, soup: BeautifulSoup
    ) -> List[JobListing]:
        ...
        for element in job_elements:
            try:
                ...
                location_elem = element.select_one(
                    ".job-location, .opening-job-location"
                )
                location = (
                    location_elem.get_text(strip=True) if location_elem else "Remote"
                )
    ...
    def _extract_ashby_jobs(
        ...
        for element in job_elements:
            try:
                title_elem = element.select_one(
                    'h3, .job-title, [data-testid*="title"]'
                )
                title = title_elem.get_text(strip=True) if title_elem else ""
    ...
    def _extract_job_from_element(
        self, company_name: str, uri: str, element
    ) -> Optional[JobListing]:
        ...
        try:
            ...
            is_job_title = any(
                re.search(pattern, text_lower) for pattern in job_patterns
            )
    ...
    async def crawl_all_companies(self) -> List[JobListing]:
        ...
        async with aiohttp.ClientSession(
            ...
            for i in range(0, total_companies, batch_size):
                ...
                logger.info(
                    f"📦 Processing batch {i//batch_size + 1} ({len(batch)} companies)"
                )
                ...
                for idx, result in enumerate(batch_results):
                    if idx >= len(batch):  # Safety check
                        logger.warning(
                            f"Result index {idx} exceeds batch size {len(batch)}"
                        )
                        continue
                    ...
                    try:
                        if isinstance(result, list):
                            ...
                            failed_companies.append(
                                {"name": company_name, "error": str(result)}
                            )
                            ...
                            logger.debug(
                                f"⚠️ {company_name}: Unexpected result type {type(result)}"
                            )
                        ...
                        logger.debug(
                            f"⚠️ {company_name}: Processing error {str(processing_error)}"
                        )
                ...
                logger.info(
                    f"📊 Progress: {processed}/{total_companies} companies processed ({len(all_jobs)} jobs found so far)"
                )
        ...
        logger.info(
            f"📈 Successfully processed: {success_count}/{total_companies} companies"
        )
        ...
        if successful_companies > 0:
            ...
            top_companies = sorted(
                company_job_counts.items(), key=lambda x: x[1], reverse=True
            )[:5]
        ...
        if failed_companies:
            ...
            logger.info(
                f"⚠️ Sample failures (first {len(sample_failures)} of {failed_count}):"
            )
        ...
        self.last_crawl_summary = {
            ...
            "top_companies": (
                ...
                if "company_job_counts" in locals()
                else {}
            ),
            "sample_failures": [
                {"name": f["name"], "error": f["error"][:100]}
                for f in failed_companies[:10]
            ],
        }
    ...
    def save_jobs_to_database(self, jobs: List[JobListing]):
        ...
        try:
            ...
            logger.info(
                f"💾 Database save completed: {new_jobs} new jobs, {updated_jobs} updated jobs, {new_companies} new companies, {updated_companies} updated companies"
            )
        ...
        for selector in job_selectors:
            try:
                ...
                if elements:
                    ...
                    if filtered_jobs:
                        logger.debug(
                            f"Found {len(filtered_jobs)} jobs with selector '{selector}'"
                        )
