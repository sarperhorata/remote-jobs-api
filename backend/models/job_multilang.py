...
class MultiLangText(BaseModel):
    ...
    translations: Dict[str, str] = Field(
        default_factory=dict, description="Translations by language code"
    )
...
class JobMultiLang(BaseModel):
    ...
    def get_localized_job(self, lang: str = "en") -> dict:
        ...
        return {
            ...
            "requirements": (
                self.requirements.get_text(lang) if self.requirements else None
            ),
            ...
            "available_languages": [self.source_lang]
            + list(self.title.translations.keys()),
        }
    ...
    async def translate_job(job: JobMultiLang, target_lang: str = "en") -> bool:
        ...
        try:
            ...
            if job.requirements and target_lang not in job.requirements.translations:
                translated_req = translator.translate(
                    job.requirements.original, dest=target_lang
                )
            ...
            if job.benefits and target_lang not in job.benefits.translations:
                translated_benefits = translator.translate(
                    job.benefits.original, dest=target_lang
                )
...
class JobMigrationService:
    ...
    async def migrate_existing_job(existing_job: dict) -> JobMultiLang:
        ...
        multilang_job = JobMultiLang(
            ...
            title=MultiLangText(original=title, original_lang=source_lang),
            description=MultiLangText(original=description, original_lang=source_lang),
            requirements=(
                MultiLangText(
                    original=existing_job.get("requirements", ""),
                    original_lang=source_lang,
                )
                if existing_job.get("requirements")
                else None
            ),
