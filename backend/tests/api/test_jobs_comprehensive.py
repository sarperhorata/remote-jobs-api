...
class TestJobsComprehensive:
    ...
    def test_search_jobs_basic_query(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Python Developer", "company": "Tech Corp"}
        ]
    ...
    def test_search_jobs_experience_filter(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Senior Developer", "experience_level": "senior"}
        ]
    ...
    def test_search_jobs_grouped(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Python Developer", "company": "Tech Corp"},
            {"_id": str(ObjectId()), "title": "Python Developer", "company": "Another Corp"}
        ]
        
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.find.return_value.limit.return_value.to_list.return_value = mock_jobs
    ...
    def test_search_job_titles(self, client: TestClient):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.aggregate.return_value.to_list.return_value = mock_titles
    ...
    def test_search_companies(self, client: TestClient):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.aggregate.return_value.to_list.return_value = mock_companies
    ...
    def test_search_locations(self, client: TestClient):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.aggregate.return_value.to_list.return_value = mock_locations
    ...
    def test_get_recent_jobs(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Recent Job", "created_at": datetime.now()}
        ]
        
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.find.return_value.sort.return_value.limit.return_value.to_list.return_value = mock_jobs
    ...
    def test_get_job_recommendations(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Recommended Job", "score": 0.9}
        ]
    ...
    def test_get_jobs_with_filters(self, client: TestClient):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            mock_db.return_value.jobs.find.return_value.skip.return_value.limit.return_value.sort.return_value.to_list.return_value = mock_jobs
            mock_db.return_value.jobs.count_documents.return_value = 1
    ...
    def test_apply_for_job_success(self, client: TestClient, auth_headers):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            ...
            mock_db.return_value.job_applications.insert_one.return_value.inserted_id = ObjectId()
    ...
    def test_get_saved_jobs(self, client: TestClient, auth_headers):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Saved Job"}
        ]
    ...
    def test_search_jobs_complex_filters(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Senior Python Developer", "isRemote": True}
        ]
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db:
            ...
            response = client.get("/api/v1/jobs/search?q=python&work_type=remote&experience=senior&job_type=full-time&salary_range=80000-120000")
    ...
    def test_search_jobs_date_range_filter(self, client: TestClient):
        ...
        mock_jobs = [
            {"_id": str(ObjectId()), "title": "Recent Job", "posted_date": datetime.now()}
        ]
    ...
    def test_job_application_form_scraping(self, client: TestClient, auth_headers):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db, \
             patch('backend.routes.jobs.JobScrapingService') as mock_scraper:
            mock_db.return_value.jobs.find_one.return_value = {
                "_id": ObjectId(job_id),
                "title": "Test Job"
            }
    ...
    def test_automated_job_application(self, client: TestClient, auth_headers):
        ...
        with patch('backend.routes.jobs.get_async_db') as mock_db, \
             patch('backend.routes.jobs.AutoApplicationService') as mock_auto_apply:
            mock_db.return_value.jobs.find_one.return_value = {
                "_id": ObjectId(job_id),
                "title": "Test Job",
