    ...
    for job in jobs:
        ...
        if result.modified_count > 0:
            ...
            if updated_count <= 5:  # Show first 5 examples
                print(f"Updated job {job_id}: '{current_company[:50]}...' -> '{new_company_name}'")
    
    print(f"\nSuccessfully updated {updated_count} jobs")
    
    # Also fix any other problematic entries
    other_problematic = db.jobs.find({"company": {"$regex": "linkedintwitter|instagrampodcast|icon.*icon", "$options": "i"}})
    other_count = 0
