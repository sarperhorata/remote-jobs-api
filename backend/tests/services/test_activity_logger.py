...
from models.user_activity import ActivityType, UserActivity, UserSession
...
class TestActivityLogger:
    ...
    def mock_db(self):
        ...
        db.user_activities.insert_one = AsyncMock(
            return_value=Mock(inserted_id=ObjectId())
        )
        db.user_activities.find = Mock(
            return_value=Mock(
                sort=Mock(
                    return_value=Mock(
                        skip=Mock(
        ...
        db.user_activities.aggregate = Mock(
            return_value=Mock(to_list=AsyncMock(return_value=[]))
        )
    ...
    async def test_log_activity_mock_database(self):
        ...
        activity_id = await logger.log_activity(
            activity_type=ActivityType.LOGIN, user_id="user123"
        )
    ...
    async def test_start_session(self, logger_service, mock_db):
        ...
        session_id = await logger_service.start_session(
            user_id="user123", session_token="token123", request_data=request_data
        )
    ...
    async def test_get_user_activities(self, logger_service, mock_db):
        ...
        mock_find = Mock()
        mock_find.sort.return_value.skip.return_value.limit.return_value.to_list = (
    ...
    async def test_get_activity_analytics(self, logger_service, mock_db):
        ...
        result = await logger_service.get_activity_analytics(
            user_id="user123", start_date=start_date
        )
    ...
    async def test_update_session_activity(self, logger_service, mock_db):
        ...
        await logger_service._update_session_activity(
            session_id, ActivityType.JOB_SEARCH
        )
        ...
        assert (
            "$set" in update_data
            or "last_activity" in str(update_data)
            or "$set" in str(update_data)
        )
    ...
    async def test_update_session_activity_error(self, logger_service, mock_db):
        ...
        await logger_service._update_session_activity(
            "session123", ActivityType.JOB_SEARCH
        )
    ...
    async def test_update_activity_summary(self, logger_service, mock_db):
        ...
        assert (
            "user_id" in str(filter_data)
            or user_id in str(filter_data)
            or "activity_type" in str(filter_data)
        )
