...
class SkillsExtractionService:
    ...
    async def extract_skills_from_cv(
        self, cv_content: str, user: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            skills_with_confidence = self._calculate_confidence_scores(
                categorized_skills, cv_content
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "technical_count": len(
                        [
                            s
                            for s in skills_with_confidence
                    ...
                    "certifications_count": len(
                        [
                            s
                            for s in skills_with_confidence
                            if s.get("category") == "certifications"
                        ]
                    ),
                },
        ...
        try:
            ...
            response = await openai.ChatCompletion.acreate(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional CV parser specializing in skill extraction. Return only valid JSON.",
                    },
    ...
    def _merge_skills(
        self, ai_skills: List[Dict], pattern_skills: List[Dict]
    ) -> List[Dict[str, Any]]:
    ...
    def _determine_category(self, skill_name: str) -> str:
        ...
        if any(
            ...
            for keyword in ["tool", "platform", "software", "system", "service"]
        ):
            return "tools"
    ...
    def _calculate_confidence_scores(
        self, skills: List[Dict[str, Any]], cv_content: str
    ) -> List[Dict[str, Any]]:
        """Calculate confidence scores for extracted skills"""
        for skill in skills:
            if "confidence" not in skill:
                skill["confidence"] = self._calculate_skill_confidence(
                    skill, cv_content
                )
    ...
    def _calculate_skill_confidence(
        self, skill: Dict[str, Any], cv_content: str
    ) -> int:
        ...
        if skill_name in content_lower:
            ...
            if any(
                ...
                for section in ["skills:", "technologies:", "tools:", "languages:"]
            ):
                confidence += 10
            ...
            if any(
                ...
                for word in ["experience", "worked with", "used", "implemented"]
            ):
                confidence += 5
            ...
            if any(
                word in content_lower
                for word in ["project", "developed", "built", "created"]
            ):
                confidence += 5
    ...
    async def _fallback_extraction(self, cv_content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "soft_count": len(
                    [s for s in skills_with_confidence if s.get("category") == "soft"]
                ),
                "languages_count": len(
                    [
                        s
                ...
                "tools_count": len(
                    [s for s in skills_with_confidence if s.get("category") == "tools"]
                ),
                "certifications_count": len(
                    [
                        s
...
async def extract_skills_from_cv(
    ...
    try:
        ...
        # Extract skills
        extraction_result = await skills_service.extract_skills_from_cv(
            cv_content, user
        )
        ...
        logger.info(
            f"Skills extracted successfully for user {current_user.id}: {len(extracted_skills)} skills found"
        )
...
async def get_skill_suggestions(
    query: str = "", limit: int = 10, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        # Filter by query
        suggestions = [skill for skill in all_skills if query.lower() in skill.lower()][
            :limit
        ]
