...
class PriorityTestRunner:
    ...
    def run_category(self, category_name, category_info):
        ...
        for test_file in category_info["files"]:
            ...
            if result["status"] == "passed":
                ...
                print(
                    f"✅ {test_file} - PASSED ({result['tests_passed']} tests, {result['duration']:.2f}s)"
                )
            elif result["status"] == "failed":
                category_results["summary"]["files_failed"] += 1
                print(
                    f"❌ {test_file} - FAILED ({result['tests_failed']} failed, {result['duration']:.2f}s)"
                )
        ...
        print(
            f"   Files: {summary['files_passed']} passed, {summary['files_failed']} failed, {summary['files_skipped']} skipped"
        )
        print(
            f"   Tests: {summary['total_passed']} passed, {summary['total_failed']} failed"
        )
    ...
    def run_all_tests(self):
        ...
        sorted_categories = sorted(
            self.test_categories.items(), key=lambda x: x[1]["priority"]
        )
        ...
        for category_name, category_info in sorted_categories:
            try:
                ...
                if (
                    category_info["critical"]
                    and category_results["summary"]["files_failed"] > 0
                ):
                ...
                self.test_results[category_name] = {"error": str(e), "status": "error"}
    ...
    def generate_report(self, total_duration):
        ...
        print(
            f"📁 Files: {total_files_passed} passed, {total_files_failed} failed, {total_files_skipped} skipped"
        )
        ...
        for category_name, category_results in self.test_results.items():
            if "summary" in category_results:
                ...
                print(
                    f"   {status_icon} {category_name}: {summary['files_passed']}/{summary['total_files']} files passed"
                )
        ...
        report_data = {
            ...
            "statistics": {
                ...
                "success_rate": (
                    (total_tests_passed / total_tests * 100) if total_tests > 0 else 0
                ),
            },
