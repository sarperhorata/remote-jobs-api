...
class TestRealisticDataIntegration:
    ...
    async def test_job_search_with_realistic_data(self, client_with_realistic_data):
        ...
        response = client_with_realistic_data.get("/api/v1/jobs/search?q=Product Manager")
        assert response.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_companies_endpoint_with_realistic_data(self, client_with_realistic_data):
    ...
    @pytest.mark.asyncio
    async def test_job_autocomplete_with_realistic_data(self, client_with_realistic_data):
        """Gerçekçi veriler ile iş autocomplete testi"""
        # Job titles search endpoint kullan
        response = client_with_realistic_data.get("/api/v1/jobs/job-titles/search?q=Product")
        assert response.status_code == 200
        ...
        # "Developer" araması
        response = client_with_realistic_data.get("/api/v1/jobs/job-titles/search?q=Developer")
        assert response.status_code == 200
    ...
    async def test_job_filtering_with_realistic_data(self, client_with_realistic_data):
        ...
        # Senior seviye işler
        response = client_with_realistic_data.get("/api/v1/jobs?experience_level=Senior")
        assert response.status_code == 200
    ...
    async def test_job_sorting_with_realistic_data(self, client_with_realistic_data):
        ...
        response = client_with_realistic_data.get("/api/v1/jobs?sort=salary_max&order=desc")
        assert response.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_job_recommendations_with_realistic_data(self, client_with_realistic_data):
        ...
        response = client_with_realistic_data.get("/api/v1/jobs/recommendations?limit=5")
        assert response.status_code == 200
