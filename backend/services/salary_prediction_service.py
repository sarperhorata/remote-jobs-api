...
class SalaryPredictionService:
    ...
    def predict_salary(
        ...
        try:
            ...
            adjusted_salary = self._apply_multipliers(
                base_salary, resume_data, job_data
            )
            ...
            return {"error": str(e), "predicted_at": datetime.now().isoformat()}

    def _calculate_base_salary(
        ...
        base_salary = self.base_salaries.get(
            experience_level, self.base_salaries["mid"]
        )
    ...
    def _get_industry_multiplier(self, job_data: Dict[str, Any]) -> float:
        ...
        for industry, multiplier in self.industry_multipliers.items():
            if industry in title or industry in description or industry in company:
                return multiplier
    ...
    def _generate_market_comparison(
        self,
        predicted_salary: Dict[str, float],
        market_data: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "prediction_vs_market": {
                ...
                "percentage_diff": (
                    ((predicted_avg - market_avg) / market_avg) * 100
                    if market_avg > 0
                    else 0
                ),
    ...
    def _get_location_impact(self, job_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "multiplier": multiplier,
            "impact": (
                "High" if multiplier > 1.2 else "Medium" if multiplier > 0.8 else "Low"
            ),
        }
    ...
    def _get_skill_impact(self, resume_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "impact": (
                "High"
                if len(premium_skills) > 3
                else "Medium" if len(premium_skills) > 1 else "Low"
            ),
        }
    ...
    def _get_education_impact(self, resume_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "multiplier": multiplier,
            "impact": (
                "High" if multiplier > 1.1 else "Medium" if multiplier > 0.9 else "Low"
            ),
        }
    ...
    def get_salary_insights(
        ...
        insights = {
            ...
            "top_paying_jobs": sorted(
                predictions, key=lambda x: x["predicted_salary"]["avg"], reverse=True
            )[:5],
    ...
    def _analyze_location_salaries(
