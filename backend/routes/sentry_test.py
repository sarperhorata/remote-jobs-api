...
async def test_sentry_error(
    error_type: str = Query(
        "exception", description="Type of error to test: exception, http, logging"
    )
) -> Dict[str, Any]:
    """
    ...
    try:
        if error_type == "exception":
            ...
            # Test HTTP error
            raise HTTPException(
                status_code=500, detail="Test HTTP error for Sentry integration"
            )

        elif error_type == "logging":
            # Test logging error
            logger.error(
                "Test error log for Sentry",
                extra={
                    "test_data": {
                        "user_id": "test_user",
                        "action": "sentry_test",
...
async def test_sentry_message(
    message: str = Query(
        "Test message from Buzz2Remote API",
        description="Custom message to send to Sentry",
    )
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to send message to Sentry: {str(e)}"
        )
        ...
        if client:
            return {
                ...
                "integrations": [
                    ...
                    for integration in client.integrations.values()
                ],
            }
        else:
            return {
