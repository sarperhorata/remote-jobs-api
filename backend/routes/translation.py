...
from ..database.db import get_async_db, get_db
...
async def detect_language(text: str, current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        if translation_result["needs_translation"]:
            ...
            update_doc = {
                ...
                "updated_at": translation_result["translation_metadata"][
                    "translated_at"
                ],
            }

            await db.jobs.update_one({"_id": ObjectId(job_id)}, {"$set": update_doc})
...
async def batch_translate_jobs(
    ...
    try:
        ...
        translation_results = await translation_service.batch_translate_jobs(
            jobs_to_translate, batch_size=batch_size
        )
                ...
                translated_jobs.append(
                    {
                        ...
                        "original_language": result["original_language"],
                        "status": "translated",
                        "translation_quality": result["translation_metadata"].get(
                            "quality_score", 0
                        ),
                    }
                )
        ...
        return {
            ...
            "newly_translated": len(
                [j for j in translated_jobs if j["status"] == "translated"]
            ),
            "no_translation_needed": len(
                [j for j in translated_jobs if j["status"] == "no_translation_needed"]
            ),
...
async def get_translation_stats(
    db=Depends(get_async_db), current_user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if translated_jobs > 0:
            success_rate = (
                (translated_jobs - failed_translations) / translated_jobs
            ) * 100
        ...
        return {
            ...
            "translation_percentage": (
                (translated_jobs / total_jobs * 100) if total_jobs > 0 else 0
            ),
...
async def auto_translate_new_jobs(
    max_jobs: int = Query(
        20, le=100, description="Maximum number of jobs to translate"
    ),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        for i, result in enumerate(translation_results):
            ...
            if result.get("needs_translation"):
                ...
                await db.jobs.update_one({"_id": job["_id"]}, {"$set": update_doc})
                translated_count += 1
...
async def get_job_translation_history(
    ...
    try:
        ...
        return {
            ...
            "original_data": (
                job.get("original_data") if job.get("original_data") else None
            ),
        }
...
async def get_jobs_requiring_translation(
    ...
    language_filter: Optional[str] = Query(
        None, description="Filter by detected language"
    ),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        async for job in jobs_cursor:
            ...
            title_language, confidence = await translation_service.detect_language(
                job.get("title", "")
            )
