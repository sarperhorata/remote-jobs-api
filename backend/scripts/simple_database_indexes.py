    ...
    try:
        ...
        try:
            ...
            await jobs.create_index(
                [("title", "text"), ("company", "text")], name="title_company_text"
            )
            await jobs.create_index(
                [("is_active", 1), ("posted_date", -1)], name="active_posted_compound"
            )
        ...
        try:
            await companies.create_index(
                [("name", 1)], name="company_name_asc", unique=True, sparse=True
            )
            ...
            await companies.create_index(
                [("is_verified", 1)], name="company_verified_asc"
            )
        ...
        try:
            ...
            await users.create_index(
                [("username", 1)], name="user_username_asc", unique=True, sparse=True
            )
        ...
        try:
            ...
            await applications.create_index(
                [("user_id", 1), ("job_id", 1)],
                name="app_user_job_compound",
                unique=True,
            )
            await applications.create_index([("status", 1)], name="app_status_asc")
            await applications.create_index(
                [("applied_date", -1)], name="app_applied_date_desc"
            )
        ...
        try:
            await activity_logs.create_index(
                [("user_id", 1)], name="activity_user_id_asc"
            )
            await activity_logs.create_index(
                [("timestamp", -1)], name="activity_timestamp_desc"
            )
            await activity_logs.create_index(
                [("timestamp", 1)], name="activity_ttl", expireAfterSeconds=2592000
            )  # 30 days TTL
            await activity_logs.create_index(
                [("activity_type", 1)], name="activity_type_asc"
            )
