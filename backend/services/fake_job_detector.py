...
class FakeJobDetector:
    ...
    def _initialize_openai(self):
        ...
        if not api_key:
            logger.info(
                "OpenAI API key not configured - AI fake job detection disabled (rule-based detection still active)"
            )
            return
...
"""
    ...
    def _check_job_requirements(self, job_data: Dict[str, Any]) -> float:
        ...
        if "no experience" in requirements and self._has_high_salary_claim(job_data):
            risk += 0.2
    ...
    def _check_salary_claims(self, job_data: Dict[str, Any]) -> float:
        ...
        hourly_matches = re.findall(
            r"\$(\d+)\s*per\s*hour", salary_text + " " + description
        )
    ...
    def _combine_analysis(
        ...
        recommendation = self._generate_recommendation(
            risk_level, final_score, ai_results
        )
    ...
    def _generate_recommendation(
        self, risk_level: FakeJobRiskLevel, score: float, ai_results: Dict[str, Any]
    ) -> str:
    ...
    async def get_analysis_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            results = await db.fake_job_analyses.aggregate(pipeline).to_list(
                length=None
            )
