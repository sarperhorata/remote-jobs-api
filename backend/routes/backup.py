...
async def create_full_backup(
    background_tasks: BackgroundTasks, api_key: str = Depends(require_api_key)
) -> Dict[str, Any]:
    """
...
async def create_incremental_backup(
    since_days: int = 1, api_key: str = Depends(require_api_key)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Incremental backup operation failed: {str(e)}"
        )


@router.get("/list")
async def list_backups(api_key: str = Depends(require_api_key)) -> Dict[str, Any]:
    """
...
async def restore_backup(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Restore operation failed: {str(e)}"
        )
...
async def cleanup_old_backups(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Cleanup operation failed: {str(e)}"
        )


@router.get("/status")
async def get_backup_status(api_key: str = Depends(require_api_key)) -> Dict[str, Any]:
    """
...
@router.post("/test")
async def test_backup_system(api_key: str = Depends(require_api_key)) -> Dict[str, Any]:
    """
    🧪 Test backup system
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Backup system test failed: {str(e)}"
        )
