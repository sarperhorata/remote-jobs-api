...
class TestLinkedInCV:
    ...
    def test_linkedin_fetch_cv_not_connected(self, mock_db, mock_jwt_auth):
        ...
        mock_user = {"_id": "user_id_123", "linkedin_connected": False}
    ...
    @patch("backend.utils.linkedin.LinkedInIntegration.get_user_cv_data")
    @patch("backend.database.get_async_db")
    def test_linkedin_fetch_cv_api_failure(
        self, mock_db, mock_get_cv_data, mock_jwt_auth
    ):
...
class TestLinkedInCVIntegration:
    ...
    def test_get_user_cv_data_success(self):
        ...
        with patch.object(linkedin, "get_user_profile") as mock_profile, patch.object(
            linkedin, "_get_experience"
        ) as mock_exp, patch.object(
            linkedin, "_get_education"
        ) as mock_edu, patch.object(
            linkedin, "_get_skills"
        ) as mock_skills:
    ...
    def test_get_skills_success(self):
        ...
        mock_response.json.return_value = {
            "skills": {
                "elements": [
                    {"skill": {"name": "Python", "id": "1"}},
                    {"skill": {"name": "JavaScript", "id": "2"}},
                ]
            }
        }
