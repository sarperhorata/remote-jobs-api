...
class TestApplicationsAPI:
    ...
    def test_applications_endpoints_exist(self, client):
        ...
        response = client.post(
            ...
            json={"job_id": "test123", "application_type": "external"},
        )

        # Should return auth error (401/403), not 404 Not Found
        assert (
            response.status_code != 404
        ), f"Applications apply endpoint not found - got {response.status_code}"
        ...
        for endpoint in get_endpoints:
            ...
            assert (
                response.status_code != 404
            ), f"Endpoint {endpoint} not found - got 404"
            assert response.status_code in [
                401,
                403,
...
class TestApplicationsIntegration:
    ...
    def test_applications_router_included(self, client):
        ...
        assert (
            len(applications_paths) > 0
        ), "No applications endpoints found in OpenAPI spec"
