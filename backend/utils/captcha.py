...
class CaptchaVerifier:
    ...
    async def verify_token(cls, token: str, remote_ip: Optional[str] = None) -> bool:
        ...
        if os.getenv("CAPTCHA_SKIP_VERIFICATION") == "true":
            logger.warning(
                "CAPTCHA verification skipped due to CAPTCHA_SKIP_VERIFICATION=true"
            )
            return True
    ...
    async def verify_recaptcha(
        cls, token: str, remote_ip: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            data = {"secret": cls.RECAPTCHA_SECRET_KEY, "response": token}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    cls.RECAPTCHA_VERIFY_URL, data=data
                ) as response:
                    if response.status != 200:
                        logger.error(
                            f"reCAPTCHA verification failed with status {response.status}"
                        )
                        return False
    ...
    async def verify_hcaptcha(cls, token: str, remote_ip: Optional[str] = None) -> bool:
        ...
        try:
            data = {"secret": cls.HCAPTCHA_SECRET_KEY, "response": token}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(cls.HCAPTCHA_VERIFY_URL, data=data) as response:
                    if response.status != 200:
                        logger.error(
                            f"hCaptcha verification failed with status {response.status}"
                        )
                        return False
