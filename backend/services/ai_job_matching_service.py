...
class AIJobMatchingService:
        ...
        try:
            ...
            for job in jobs:
                ...
                if match_score > 0.3:  # Minimum match threshold
                    ...
                    job['match_reasons'] = await self._get_match_reasons(user_profile, job)
    ...
    async def _get_user_profile(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Get user experience
            experience = await self.db.user_experience.find({"user_id": user_id}).to_list(None)
    ...
    async def _get_available_jobs(self, filters: Optional[Dict] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if job.get("description"):
                ...
                common_skills = [
                    ...
                    "kubernetes", "sql", "mongodb", "postgresql", "git",
                    "html", "css", "typescript", "vue", "angular", "django",
                    "flask", "fastapi", "redis", "elasticsearch", "java",
                    "golang", "rust", "c++", "machine learning", "ai",
                    "data science", "devops", "ci/cd", "terraform"
                ]
    ...
    async def _match_location(self, user_profile: Dict, job: Dict) -> float:
        ...
        try:
            ...
            preferred_remote = user_profile.get("preferences", {}).get("remote_work", False)
    ...
    async def _match_job_type(self, user_profile: Dict, job: Dict) -> float:
        ...
        try:
            preferred_type = user_profile.get("preferences", {}).get("job_type", "").lower()
    ...
    async def _match_company_size(self, user_profile: Dict, job: Dict) -> float:
        ...
        try:
            preferred_size = user_profile.get("preferences", {}).get("company_size", "").lower()
    ...
    async def _get_match_reasons(
        ...
        try:
            ...
            # Experience match
            user_exp = sum(exp.get("years", 0) for exp in user_profile.get("experience", []))
    ...
    async def get_match_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logs = await self.db.recommendation_logs.find(
                {"user_id": user_id}
            ...
            if logs and applications:
                ...
                if total_recommendations > 0:
                    analytics["success_rate"] = len(applications) / total_recommendations
    ...
    async def update_user_preferences_from_behavior(self, user_id: str):
        ...
        try:
            ...
            for app in applications:
                ...
                if company_size:
                    company_size_patterns[company_size] = company_size_patterns.get(company_size, 0) + 1
            ...
            if company_size_patterns:
                preferred_size = max(company_size_patterns, key=company_size_patterns.get)
