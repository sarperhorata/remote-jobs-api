...
from backend.schemas.user import OnboardingStep, UserCreate
from backend.utils.email import (create_email_verification_token,
...
async def register_with_email_only(
    user_data: UserCreate, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def verify_email(
    verification_data: OnboardingStep, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def set_password(
    password_data: OnboardingStep, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
    ...
    try:
        ...
        if password_data.password != password_data.confirm_password:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Şifreler eşleşmiyor"
            )
...
async def complete_profile(
    profile_data: dict, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
...
async def get_onboarding_status(
    user_id: str, db: AsyncIOMotorDatabase = Depends(get_async_db)
):
    ...
    try:
        ...
        return {
            ...
            "next_step": steps.get(
                user.get("onboarding_step", 0), "email_verification"
            ),
