...
class LoadTestRunner:
    ...
    async def run_scenario(self, scenario: LoadTestScenario) -> LoadTestResult:
        ...
        success_rate = (
            (successful_requests / len(all_results)) * 100 if all_results else 0
        )
    ...
    async def _user_workload(
        self, client: LoadTestClient, scenario: LoadTestScenario, user_id: int
    ) -> List[Dict]:
        ...
        while time.time() < end_time:
            ...
            result = await client.make_request(
                scenario.endpoint, scenario.method, scenario.payload, scenario.headers
            )
    ...
    def _generate_summary(self) -> Dict:
        ...
        return {
            ...
            "avg_requests_per_second": (
                statistics.mean(requests_per_second) if requests_per_second else 0
            ),
...
<body>
    ...
    {scenarios_html}
        ...
        for scenario in report_data["scenarios"]:
            success_class = (
                ...
                else "warning" if scenario["success_rate"] >= 80 else "error"
            )
    ...
    def _generate_charts(self, output_path: Path, timestamp: str):
        ...
        try:
            ...
            plt.bar(
                x, p95_times, width, label="95th Percentile", color="orange", alpha=0.7
            )
            plt.bar(
                [i + width for i in x],
                p99_times,
...
def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8001", help="Base URL for testing"
    )
    parser.add_argument(
        "--scenarios", default="default", help="Scenarios to run (default, custom)"
    )
    parser.add_argument(
        "--output", default="load_test_reports", help="Output directory for reports"
    )
    parser.add_argument(
