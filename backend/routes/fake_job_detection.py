...
from backend.services.fake_job_detector import (FakeJobRiskLevel,
                                                fake_job_detector)
            ...
            "recommendations": {
                "high_risk_jobs": len(
                    [
                        ...
                        for a in analyses
                        if a.risk_level
                        in [FakeJobRiskLevel.HIGH, FakeJobRiskLevel.CRITICAL]
                    ]
                ),
                "requires_review": len(
                    [a for a in analyses if "REVIEW" in a.recommendation]
                ),
                "should_reject": len(
                    [a for a in analyses if "REJECT" in a.recommendation]
                ),
            },
        }
...
async def get_job_analysis_history(
    job_id: str, admin=Depends(get_current_admin), db=Depends(get_async_db)
):
    """
    ...
    try:
        ...
        analyses_cursor = db.fake_job_analyses.find({"job_id": job_id}).sort(
            "analyzed_at", -1
        )
...
async def get_high_risk_jobs(
    ...
    try:
        ...
        analyses_cursor = (
            db.fake_job_analyses.find(query)
            .sort("analyzed_at", -1)
            .skip(skip)
            .limit(limit)
        )
...
async def bulk_action_on_jobs(
    job_ids: List[str],
    action: str = Query(
        ..., description="Action to perform: analyze, approve, reject, flag"
    ),
    admin=Depends(get_current_admin),
    db=Depends(get_async_db),
):
            ...
            for job in jobs:
                try:
                    ...
                    results["errors"].append(
                        f"Job {job.get('_id', 'unknown')}: {str(e)}"
                    )
