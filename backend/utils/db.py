...
class Database:
    ...
    def get(self, table, id):
        """
        ID'ye göre tablo kaydını döndürür
        """
    ...
    def add(self, table, data):
        """
        Tabloya veri ekler (insert'in alternatif ismi)
        """
...
def create_user(user_data):
    return db.add("users", user_data)

def update_user(user_id, user_data):
    return db.update("users", user_id, user_data)
...
client = None  # MongoClient(mongodb_settings['uri'])
db = Database()

# Define MongoDB collections
try:
    # Ekstra güvenlik için try-except bloğu içinde tanımlıyoruz
    ...
    support_tickets = db.execute("SELECT * FROM support_tickets")
    countries = db.execute("SELECT * FROM countries")
except Exception as e:
    logger.warning(f"Bazı tablolar bulunamadı: {str(e)}")
...
# Connect to async MongoDB database for real-time operations
async_client = None  # AsyncIOMotorClient(mongodb_settings['uri'])
async_db = Database()

# Define async MongoDB collections
try:
    async_users = db.execute("SELECT * FROM users")
    async_jobs = db.execute("SELECT * FROM jobs")
    async_notifications = db.execute("SELECT * FROM notifications")
    async_ads = db.execute("SELECT * FROM ads")
    async_support_tickets = db.execute("SELECT * FROM support_tickets")
except Exception as e:
    logger.warning(f"Async: Bazı tablolar bulunamadı: {str(e)}")
    # Hata durumunda boş değerler atayalım
    async_users = None
    async_jobs = None
    async_notifications = None
    async_ads = None
    async_support_tickets = None
