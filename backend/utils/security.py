...
class SecurityUtils:
    ...
    def _cleanup_old_data() -> None:
        ...
        expired_tokens = [
            token
            for token, expiry in rate_limit_data["csrf_tokens"].items()
        ...
        for ip in list(rate_limit_data["ip_violations"].keys()):
            ...
            if ip in rate_limit_data["ip_endpoint_counters"]:
                has_recent = False
                for endpoint, timestamps in rate_limit_data["ip_endpoint_counters"][
                    ip
                ].items():
        ...
        logger.info(
            f"Cleaned up security data. Active IPs: {len(rate_limit_data['ip_endpoint_counters'])}, "
    ...
    async def get_current_user(token: str = Depends(oauth2_scheme)):
        ...
        from utils.jwt import \
            decode_token  # Import here to avoid circular imports
