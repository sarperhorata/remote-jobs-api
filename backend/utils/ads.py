...
def create_campaign(
    client: GoogleAdsClient, customer_id: str, campaign_name: str, budget: float
) -> Optional[str]:
    """
    ...
    try:
        ...
        campaign_budget.amount_micros = int(
            budget * 1000000
        )  # Mikro birimlere dönüştür
...
def create_ad_group(
    client: GoogleAdsClient, customer_id: str, campaign_id: str, ad_group_name: str
) -> Optional[str]:
    """
...
def create_ad(
    ...
    try:
        ...
        responsive_search_ad.headlines.append(
            client.get_type("AdTextAsset").create(text=headline)
        )
        responsive_search_ad.descriptions.append(
            client.get_type("AdTextAsset").create(text=description)
        )
    ...
    try:
        ...
        for row in response:
            return {
                ...
                / 1000000,  # Mikro birimlerden normal birimlere dönüştür
