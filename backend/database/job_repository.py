...
class JobRepository:
    ...
    def save_job(self, job_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            result = self.collection.update_one(query, {"$set": job_data}, upsert=True)
    ...
    def get_similar_jobs(self, job_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not skills:
                query = {"_id": {"$ne": ObjectId(job_id)}, "$text": {"$search": title}}
                return self.get_jobs(query=query, limit=limit)

            # Match by skills (at least one skill must match)
            query = {"_id": {"$ne": ObjectId(job_id)}, "skills": {"$in": skills}}
    ...
    def archive_old_jobs(self, days_threshold: int = 30) -> int:
        ...
        try:
            ...
            query = {"date_posted": {"$lt": cutoff_date}, "archived": {"$ne": True}}

            # Update to mark as archived
            result = self.collection.update_many(
                query, {"$set": {"archived": True, "updated_at": datetime.utcnow()}}
            )

            archived_count = result.modified_count
            logger.info(
                f"Archived {archived_count} jobs older than {days_threshold} days"
            )
